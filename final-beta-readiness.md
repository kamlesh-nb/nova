...
- **B6** (fixed) a generic `async fn` calling `serde.bind<T>` failed three ways (erased-body `T__bind`
  ...
  name first. Case 309. (A separate PRE-EXISTING B3-family bug -- returning an owned type-param argument
  unchanged double-frees in BOTH sync and async -- remains, out of scope.)
