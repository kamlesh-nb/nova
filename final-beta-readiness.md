...
| D1/D2/D3 | D erased carriers | S-crit | 🔵 (partial) | ✅ D3 fixed: a trait→concrete downcast (`traitObj as Square`) was UNCHECKED and silently reinterpreted memory when the actual type differed (`Circle as Square` handed back a bogus Square). Now the downcast loads the trait object's vtable (which uniquely identifies the concrete type for that trait) and traps via `nova_panic_cstr` on mismatch. Case 305 (positives; the wrong-downcast trap aborts, verified manually like 277). ⬜ D1 (`any` owning-heap UAF) and D2 (any-downcast double-free) now CONFIRMED live: `let a: any = box; return a` (source scope drops) → heap-use-after-free in `nova_release`; `let a: any = src; let b = a as Box` → double-free. Root cause: storing a heap value into `any` does NOT retain it (the `any` borrows, then dangles when the source is released). `nova_any_box`/`nova_any_box_dtor` exist in the runtime but codegen never calls them — the owning-`any` model is unimplemented. Fix options: (a) box `any` as {payload, dtor}, retain on widen + release on drop via the dtor (correct, but changes `any`'s representation everywhere — reads/downcasts must unbox); (b) retain the heap value on widen-to-`any` without releasing (converts the UAF/double-free into a bounded LEAK — memory-safe stopgap). Both are multi-site; deferred pending a direction decision. ATTEMPTED the owning-any boxing (option a): added a distinct owned `any_` TypeStore variant (lowers there, isOwned=true, renders "any"), `nova_any_box`/`nova_any_unbox`/`nova_any_box_dtor` wiring, and boxing at the let-init + call-argument widen seams (`coerceValoptArg`) + unbox at the cast-from-any. The LOCAL case worked (D2 roundtrip ASAN-clean), which VALIDATES the design. Blocked on the `Storage<any>` container path: `Map<string, any>.set(k, 42)` faults in `nova_retain` (case 123 test 1) -- the monomorphised storage's element retain/release interacts with the boxed owned-any incorrectly, needs deeper ARC work on the storage element handling. Reverted the WIP (kept a broken compiler out of tree); the design + exact blocker are recorded for the next pass |
