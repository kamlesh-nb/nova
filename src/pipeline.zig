...
/// calling convention as [`kyte_lld_link_macho`]. Called by [`linkWasmInProcess`].
extern fn kyte_lld_link_wasm(argv: [*]const [*:0]const u8, argc: c_int) c_int;
...
/// [`kyte_lld_link_wasm`]; a non-zero return becomes `error.LinkFailed`.
pub fn linkWasmInProcess(allocator: std.mem.Allocator, obj_path: []const u8, output_path: []const u8) !void {
    ...
    const rc = kyte_lld_link_wasm(cargv.items.ptr, @intCast(cargv.items.len));
    if (rc != 0) {
