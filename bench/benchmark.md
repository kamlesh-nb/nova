# Kyte — Compute Benchmark Results

Cross-language comparison of **Kyte** against **Rust**, **Go**, and **C#** on six algorithms from the
[Programming Language Benchmarks](https://programming-language-benchmarks.vercel.app/) suite.

Every implementation is a **scalar, single-threaded** port of the *same* algorithm. Every Kyte program
...
The Kyte sources here use **`for (i in 0..n)` range loops** for their counted loops. That matters: a
...
Kyte here uses **`for (i in 0..n)` range loops** and the **`KYTE_ARC_ELIDE` retain/release elision** (see
below); that is the best config measured.

| Benchmark | Rust | Go | C# | **Kyte** | Fastest peer | **Kyte vs fastest** |
|---|---:|---:|---:|---:|---|---:|
| spectral-norm (N=1000) | 38.1 | 40.1 | 68.8 | **38.7** | Rust 38.1 | **1.02× (tie)** |
| fannkuch-redux (n=11) | 2094 | 1975 | 2005 | **1927** | *Kyte* | **fastest** |
...
**Bottom line: Kyte matches Rust within noise on 4 of 6 (spectral-norm, fannkuch, mandelbrot, nbody) — and
...
**Tied / within noise:**
- **mandelbrot** (1.00× vs Rust) — scalar-double escape math into a bit-packed buffer. Kyte and Rust are
...
- **nbody** (1.07× vs Rust) — scalar N-body with a `sqrt` per step. Tied once `fsqrt` was lowered to the
  hardware `llvm.sqrt.f64` intrinsic (it was ~20× slower before that fix). Kyte is 2.4× faster than Go
...
- **nsieve** (1.41× vs Rust) — boolean sieve over a fixed array. The hot work is *strided single-byte
  ...
  cache-and-branch-bound loop where Kyte's per-iteration codegen is simply looser than Rust's.
- **binary-trees** (2.20× vs C#) — allocation churn. Kyte's ARC (reference counting) is correct and
  ...
  remains Kyte's clear weak spot** — it is the one benchmark where the language model, not the codegen,
  is the ceiling.
...
Every Kyte result was verified against the peers at the **same problem size**, not just against a stored
expected value:
...
Kyte also has an explicit `f64x4` vector type (LLVM `<4 x double>`). On a dot-product kernel:

| | time | |
|---|---:|---|
| Kyte — scalar loop | 384 ms | baseline |
| **Kyte — f64x4 SIMD** | **98 ms** | **~4× faster than its own scalar** |

This is a **self-comparison** (Kyte-SIMD vs Kyte-scalar), *not* a win over the peers: a hand-vectorized
Rust/C# version would also be ~4×. The honest claim is only that Kyte now *has* usable, deterministic
...
1. **Strided integer memory (nsieve).** The vectorizer can't help a sieve — each stride writes one byte
   ...
   Rust/Go are still tighter than Kyte's codegen.
...
| Kyte | 0.1.0 · `KYTE_ARC_ELIDE=1 kyte <file>.ky --release` · host-CPU target, loop+SLP vectorization + ARC elision |
| Timing | best of 8–12 wall-clock runs, milliseconds |

Each benchmark folder holds the source files (`*.ky`, `*.rs`, `*.go`, `cs/`) and the reference golden
output. The Kyte sources are the `*_for.ky` / range-for variants. Ratios are Kyte divided by the
...
6. **`f64x4` SIMD** — explicit vector type (~4× over Kyte's own scalar), for kernels the vectorizer
   can't prove.
7. **`KYTE_ARC_ELIDE` retain/release elision** — a compile-time peephole that removes a defensive
   `kyte_retain` + its paired `kyte_release`(s) when a local only ever holds a value copied from a
...
capability, so effort moves to features. Kyte matches Rust within noise on 4 of 6 and beats Go/C# on
...
resolve). Every peer takes its size on argv; the Kyte sources are the tuned `*_for.ky` range-for
variants (binary-trees adds `KYTE_ARC_ELIDE=1`). Golden output was verified at each size in the SAME run
(not against a stored value): spectral `1.274224153`, fannkuch `556355` / `Pfannkuchen(11) = 51`,
mandelbrot md5 `e01bb5628cbf0254313292d48368f99b` (Kyte == C#), nbody `-0.169075164` / `-0.169031665`,
nsieve `14683/7837/4203`, binary-trees `check: 524287`. Times are best-of-6 user-seconds, lower is better.

| Benchmark | Rust | Go | C# | **Kyte** | Kyte vs Rust | Fastest |
|---|---:|---:|---:|---:|---:|---|
| spectral-norm (N=5500) | 1.14 | 1.15 | 1.19 | **1.15** | **1.01× (tie)** | Rust |
| fannkuch-redux (N=11) | 2.07 | 1.95 | 1.98 | **1.92** | **0.93× (fastest)** | **Kyte** |
| mandelbrot (4000²) | 1.66 | 1.45* | 1.70 | **1.66** | **1.00× (tie)** | Rust/Kyte† |
...
like-for-like time. † Among the like-for-like peers (Rust, C#, Kyte) Kyte ties Rust for fastest.

**Reproduces the original findings exactly:** Kyte ties or beats Rust/Go/C# on the four compute-bound
...
than at 16, as allocation churn dominates). `KYTE_ARC_ELIDE` changed the binary-trees codegen but moved
