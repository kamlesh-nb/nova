...
- You will need: the `nova` compiler on your PATH, and the guide example `examples/18_decimal.nova`

## Hook (0:00)

**Say:** Here is a classic trap. In almost every language, `0.1 + 0.2` does not equal `0.3`. It equals `0.30000000000000004`, because binary floating point cannot represent those numbers exactly. That is fine for physics, but it is a disaster for money. Nova has a type that fixes it: `decimal`, exact base-10 arithmetic. By the end of this video you will use it for prices, totals, and comparisons, with no drift at all.
...
**Run it:** `nova examples/18_decimal.nova -o out && ./out`
...
**Say:** That wraps up decimal, and with it the core value types. Next we move on to ownership and memory, where we look at how Nova manages heap values without a garbage collector. If this helped, a like and a subscribe are much appreciated.
