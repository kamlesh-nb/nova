...
- You will need: Kyte installed, and the first video watched so you know how to compile and run.

## Hook (0:00)

**Say:** Kyte is statically typed, which means every value has a type the compiler knows about before your program ever runs. In this video we will walk through the scalar types, the operators you use on them, how to cast between numeric types, and the two ways to declare variables. By the end you will be comfortable writing real expressions and binding values to names.
...
**Say:** Now let us look at operators. Kyte has the arithmetic, bitwise, and shift operators you would expect. Here is the second half of the same file.
...
**Say:** Kyte does not silently convert between numeric types. When you want to convert, you say so explicitly with the `as` keyword.
...
**Run it:** `kyte docs/guide/examples/02_primitives.ky -o /tmp/primitives && /tmp/primitives`
...
**Say:** Now, variables. Kyte has exactly two binding keywords. `let` is mutable, meaning you can reassign it. `const` is immutable, and trying to reassign a `const` is a compile error. That is it, there is no `var`, it was removed. And when the compiler can work out the type for you, the annotation is optional.
...
**Run it:** `kyte docs/guide/examples/03_variables.ky -o /tmp/variables && /tmp/variables`
...
**Say:** That gives you a solid grip on Kyte's values and types. Next up we look at strings in detail: template literals, concatenation, and the string standard library. If this helped, do drop a like and subscribe. See you there.
