...
**Say:** Nova talks to a database through one interface and many drivers. The interface is the Connection seam in the standard library; each database ships as its own git package that implements it. In this video we tour all four: PostgreSQL, MySQL, SQL Server, and MongoDB. For each you will see the one command that adds it, the import, the connection string, and how you connect. Because every SQL driver speaks the same seam, the query code is identical whichever engine you point at.
...
**Say:** The same Pool backs every driver: PgDriver, MyDriver, MssqlDriver, MongoDriver, no per-driver pool. Now the drivers, one at a time. The pattern is always the same three lines: nova get the URL, import the module, connect with a URL.
...
**Say:** Four drivers, one seam. Each is a git package you add with nova get, import by its module name, and connect with a URL. The three SQL drivers share the same query code; MongoDB brings a document API. Put a pool in front for a real server, and write your data access against the Connection trait so none of it cares which database is underneath. Next we take a database-backed app and deploy it.
