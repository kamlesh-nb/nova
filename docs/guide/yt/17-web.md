...
```
Features/Products/
  routes.ky                 this feature's routes
  CreateProduct/command.ky  write input   (a @serializable struct)
  CreateProduct/handler.ky  the RouteHandler
  CreateProduct/validator.ky
...
**Say:** Curly-brace-id-colon-int is a typed path parameter; a non-numeric id is a 400 at the router and never reaches the handler. And main.ky is the composition root: it builds the shared dependencies once and calls each feature's register. No container resolving things behind your back; you see every dependency constructed.
...
**Say:** The project also has main_postgres.ky at its root: the same app over a real PostgreSQL. Put it next to src/main.ky and only the composition root differs. Every slice, the repository, the handlers, the views are identical, because they depend on the Connection trait, never on a driver.
