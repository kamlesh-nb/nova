...
- You will need: Nova installed, and the previous two videos watched.

## Hook (0:00)

**Say:** Strings are everywhere in real programs, so let us get comfortable with them. In this video you will learn what a Nova string actually is, the idiomatic way to build strings with template literals, and the string standard library that gives you things like uppercasing, slicing, and searching. By the end you will be formatting and manipulating text like it is second nature.
...
**Say:** At the top, `import string;` brings in the standard string functions. Inside `main`, `a` and `b` are two string literals. The first `console.log` shows concatenation with the plus operator, joining `"Hello"`, `", "`, `"Nova"`, and `"!"` into one string. The second line shows two things: a template literal with `${a}` interpolated inside quotes, and `a.length`, which is the byte length of the string. Note that `length` is a property, not a method call, so there are no parentheses after it.
...
**Say:** Here `n` is 7 and `ok` is true. In the template we interpolate `n` directly, we interpolate the boolean `ok`, and then `${n / 2}` is an actual expression evaluated inside the braces. Since `n` is an integer, `n / 2` is integer division, so half comes out as 3, not 3.5. This is why template literals are the canonical formatter in Nova: they handle the stringifying and let you drop expressions right in.
...
**Say:** `string.toUpperCase` uppercases the text, so `"nova"` becomes `NOVA`. `string.slice` takes a start and an end and returns the substring in that half-open range, so slicing `"hypermedia"` from 0 to 3 gives `hyp`. `string.indexOf` returns the first index where a substring appears, or -1 if it is not found, so `"per"` is found at index 2. And `string.contains` is a simple yes or no, does the string contain this substring, so `"media"` gives true.

**Run it:** `nova docs/guide/examples/04_strings.nova -o /tmp/strings && /tmp/strings`
...
**Say:** Everything lines up. The concatenation produced `Hello, Nova!`, the length of `Hello` is 5, half of 7 is 3, uppercasing gave NOVA, the slice gave hyp, indexOf found per at 2, and contains returned true.
