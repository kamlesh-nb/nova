...
# never enters the reactor: NOVA_REACTOR=uring changes nothing there and NOVA_IO_WATCHDOG=1 prints
# nothing, because no reactor loop is ever driven. It therefore cannot verify io_uring, no matter what
# the environment says.
#
# worker_async.nova closes that gap by registering the INHERITED descriptor with net.poller.Reactor
# and polling it before replying. Same binary both times; only NOVA_REACTOR differs, which is what
...
build() {
  nova router.nova       -o router       >/dev/null 2>&1 || { echo "router build failed";  exit 1; }
  nova worker_async.nova -o worker_async >/dev/null 2>&1 || { echo "worker build failed";  exit 1; }
}

# $1 = backend label passed as NOVA_REACTOR ("epoll" is not a recognised value -- anything that is not
# uring/io_uring falls through to epoll, which is exactly the default path we want to compare against).
run_backend() {
  local backend="$1" rc=0
  cleanup
  echo "=================================================================="
  echo " backend: NOVA_REACTOR=$backend"
  echo "=================================================================="

  NOVA_REACTOR="$backend" WORKERS=2 ./router >/tmp/nova_router_$backend.log 2>&1 &
  sleep 1.2
  NOVA_REACTOR="$backend" WORKER_ID=A ./worker_async >/tmp/nova_wa_$backend.log 2>&1 &
  NOVA_REACTOR="$backend" WORKER_ID=B ./worker_async >/tmp/nova_wb_$backend.log 2>&1 &
  sleep 1.5

  if ! grep -q "up;" /tmp/nova_router_$backend.log 2>/dev/null; then
    echo "  ROUTER FAILED TO START:"; sed 's/^/    /' /tmp/nova_router_$backend.log; return 1
  fi
  ...
  echo "--- worker A said ---"; sed 's/^/    /' /tmp/nova_wa_$backend.log
  if grep -qi "FAILED to arm" /tmp/nova_wa_$backend.log /tmp/nova_wb_$backend.log 2>/dev/null; then
  ...
  pkill -9 -x router >/dev/null 2>&1
  sleep 0.4
  echo "--- worker A on shutdown ---"; sed 's/^/    /' /tmp/nova_wa_$backend.log
  pkill -9 -x worker_async >/dev/null 2>&1
  cleanup
  return $rc
}
