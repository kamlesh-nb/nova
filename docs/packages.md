...
Nova's package model is deliberately small: a package is a **git repository** containing Nova source,
...
`nova init` writes a `project.json` at the project root:
...
```bash
nova get https://github.com/<owner>/<repo>
```

This clones the repository into the local package cache and appends its URL to `dependencies` in
`project.json`. In your Nova source you then `import` the package's modules by their module path, the
...
```bash
nova get                    # no URL: reads project.json and restores all dependencies
```
...
To publish, you make a git repository that other projects can `nova get`:

1. Put your Nova modules under a clear module path (the directory layout is the import path).
...
3. If the package needs native code, keep that build self-contained and documented; a package that
   links a native library is environment-dependent for its consumers, so prefer pure Nova where you
   can.
4. Tag releases (`vX.Y.Z`) so consumers can pin an exact version.
5. Push to a git host. Consumers add it with `nova get <your-repo-url>`.
...
The standard library (`src/std/`) ships with the compiler and is imported without a `nova get` step:
