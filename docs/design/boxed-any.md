...
(`Map<string, any>`, di.nova's singleton cache) is RETAINED by the container and freed with it — fixing the
dangling-pointer gap of the interim unowned `.ptr` representation. See `nova-any-ownership-model` (memory) for the
options considered; this is "option 1: box value types."

## Status

- ✅ **Runtime foundation LANDED + verified** (`src/runtime/alloc.cpp`, `nova_abi.h`): `nova_any_box(payload, dtor)`,
  `nova_any_unbox(box)`, `nova_any_box_dtor(box)`. Zero behavior change until codegen wires them.
...
A box is a normal ARC object (`nova_bytes_alloc(16)`, rc=1, 8-byte header at `[ptr-8]`):
...
- Boxing **MOVES** the payload's +1 into the box (no extra retain). Releasing the box calls `nova_any_box_dtor`,
  which `nova_release`s the inner value via `dtor` (if non-0); the box's own bytes are then freed by the
  `nova_release(box, nova_any_box_dtor)` that invoked it.
...
7. **Destructor for `.any`**: `getOrCreateDestructorByTypeId(.any)` → the runtime `nova_any_box_dtor` (register it
   in `func_map`, `declarations.zig`, like the other `nova_*` intrinsics). So when a container element / local /
   field of type `any` is released, ARC calls `nova_any_box_dtor`, which frees the inner value.
8. **Register intrinsics** (`declarations.zig`): `nova_any_box` (i64,i64→i64), `nova_any_unbox` (i64→i64),
   `nova_any_box_dtor` (i64→void) in `func_map`.
...
`nova_any_box(value, <dtor-of-T or 0>)` where the dtor is `getOrCreateDestructor(T)` when `isOwned(T)`, else 0.
The four categories (the same insertion points trait-widening uses):

- **(a) Return coercion** — a fn/closure whose declared return is `any` returning a concrete `T`. **This is di.nova's
...
- **`.cast` handler** (`expressions.zig` ~3698): if the source expr's type is `any` and the target is a concrete
  type (not `any`/`void`), emit `nova_any_unbox(val)` and reinterpret as `T`. One site. (Optionally, later: check
...
`nova_any_unbox` reads a non-box word → garbage/crash. The corpus flagship cases (56/58) + di (`123`) + ASAN are the
...
- [x] Runtime `nova_any_box`/`_unbox`/`_box_dtor` (landed, verified compiling).
- [ ] `.any` variant (owned) + lower `any`→`.any` + exhaustive-switch ripple resolved.
- [ ] Box at (a)-(d); unbox at `.cast`; `.any` destructor = `nova_any_box_dtor`.
