...
| M10 | Crypto namespace reorganization | M9 (soft) | TODO | The `src/std/crypto/` flat namespace has grown unwieldy (~25 modules). Reorganize into subpackages, Zig/Go-style: `crypto/hash/{sha256,sha512,sha1,md5}`, `crypto/mac/hmac`, `crypto/kdf/{hkdf,pbkdf2}`, `crypto/aead/{aesgcm,chachapoly}`, `crypto/cipher/{aes,aesctr,chacha20}`, `crypto/mac/{ghash,poly1305}`, `crypto/ecc/{p256,x25519}`, `crypto/{rsa}`, `crypto/x509`, `crypto/tls/{tls13,tls13hs,tls13client,tls13server,tls12client}`, `crypto/{base64,pem,random,scram}`. Mechanical but sweeping: move files, update every `import crypto.X`, re-register in `src/main.zig` std_modules, fix conformance-case imports. Do this FIRST so subsequent milestones (M11-M13) land in the clean structure. Reference: Zig `lib/std/crypto/*` and Go `crypto/*` layout |
| M11 | TlsMemBio + full wolfSSL retirement | M9, M10 | TODO | Two parts. (a) A memory-BIO TLS interface on the Nova stack: a `TlsMemBio` wrapping tls13client/tls13server's record layer + `feedHandshake` that exposes `feed(ciphertext)` / `pull(ciphertext)` / `pendingOut()` decoupled from the socket - because the MSSQL/TDS driver tunnels the TLS handshake inside TDS PRELOGIN packets (it feeds/pulls ciphertext over a custom framing, NOT TLS-over-raw-socket; this is why the naive socket-only asynctls rewrite broke cases 100/110). Then rewire BOTH `net/asynctls` (socket path, live-verified vs openssl) and the TDS driver onto it. (b) Replace the remaining wolfCrypt C surface in `src/runtime/crypto.cpp` - `nova_random_hex` (over OS `getrandom`/`urandom`), `nova_sha256/512/1`, `nova_hmac_sha256(_raw)`/`nova_sha256_raw`, `nova_md5`, `nova_pbkdf2_hmac_sha256`, `nova_mysql_scramble`/`nova_mysql_sha2_scramble`, `nova_rsa_oaep_encrypt` - with Nova (the old `crypto/{sha,md5,random}` and scram/mysql modules rewired to the Nova hash/hmac modules). THEN delete `nova_mtls_*` from `io.cpp` and drop `deps/wolfssl` + `-DNOVA_HAVE_WOLFSSL` from `build.zig`. GOTCHA: `zig build` rsyncs `src/std`->`~/.nova/std` WITHOUT `--delete`, so deleted modules linger and can collide (the dead `net/tls.nova` stub's `TlsStream_init` collides with asynctls's; delete the stale synced copy). This is the milestone that actually removes wolfSSL |
...
| M13 | TLS 1.2 client (full spec) | M11 | TODO | Full-spec TLS 1.2 CLIENT ONLY (to reach external services that require TLS 1.2). A separate protocol from 1.3: the RFC 5246 handshake (ClientHello -> ServerHello + Certificate + ServerKeyExchange + ServerHelloDone -> ClientKeyExchange + ChangeCipherSpec + Finished), the PRF (P_SHA256) key derivation, ECDHE and RSA key exchange, and the record layer with explicit-nonce AEAD (AES-GCM) and CBC+HMAC cipher suites. Reuses the Nova primitives (SHA/HMAC/AES-GCM/ECDHE/X.509). Reference: tls12.xargs.org (byte-level walkthrough), RFC 5246. Client only; no TLS 1.2 server |
