...
| M12 | Full-spec TLS 1.3 (client + server) + CA trust store | M11 | IN PROGRESS | Harden the pure-Nova TLS 1.3 stack to full RFC 8446. **DONE (M12-A): the CA trust store** - `crypto/tls/truststore` builds a TrustStore from an in-memory PEM bundle (loadPemBundle) and a trusts() decision accepts a server certificate_list iff it validates against >=1 root, for a hostname, at UTC-now, reusing the verified x509 chain logic (sig checks, validity, CA constraints, name chaining, SAN incl. wildcards). Pure (base64 + x509 + datetime, no os.sys). `nowUtc()` is civil-from-Unix (UTC regardless of TZ). Wired into `net/tlsmembio.newClientBioVerify(host, store)` (the ClientBio fails the handshake if the chain is untrusted) and exposed as `web/client.Http.getVerified(url, caPem)`. Live-verified: a real OpenSSL EC chain (Nova Test Root CA + localhost leaf) is ACCEPTED against the root store and REJECTED against an empty/wrong store, both via the raw ClientBio and via an HTTPS GET (correct CA returns the body, wrong CA fails the handshake). Conformance case 236. NOTE: web/client can't auto-load the SYSTEM bundle yet (importing io/file pulls os.sys, whose socket/close externs pollute web/client's namespace - blocked on the os.socket split), so the caller passes the bundle PEM. **M12-B (started): Alert protocol DONE** - inbound alerts (inner content type 0x15) in the client and server bios now report end-of-stream (readApp returns -1) instead of being silently skipped, and `TlsClient.encryptAlert` + `ClientBio.closeNotify()` send an encrypted close_notify at shutdown. Offline case 237 drives a full in-memory handshake then a client close_notify the server detects. **Still TODO (M12-B)**: ALPN, KeyUpdate, multi-suite/group negotiation (AES-128/256-GCM + ChaCha20-Poly1305; secp256r1 + x25519 + secp384r1), HelloRetryRequest handling (currently only rejected), NewSessionTicket + resumption / 0-RTT PSK, record-size limits. Already done (earlier): SNI, psk_key_exchange_modes, middlebox-compat CCS, HRR/downgrade rejection |
