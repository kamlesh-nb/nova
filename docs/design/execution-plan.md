...
| **D8** | ✅ **READ SIDE DONE** — `RowSource impl ValueSource` + `queryAs<T>`/`queryOne<T>` materialize typed rows via `<Struct>__bind` (params out-of-band). Write side + Mongo DocSource + live gate deferred | ⭐user P3 | ◑ | `159_micro_orm` | 159/159, ASAN 289/289 |
