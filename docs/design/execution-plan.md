...
| **I1** | **Nova reverse proxy + load balancer + PID autoscaler** (⭐"most important", user) — L4/L7 proxy on the async runtime + TCP server + **HAProxy-style per-reactor lock-free conn pool**, now **share-nothing MULTI-CORE via N1** (per-reactor SO_REUSEPORT accept fan-out; `Proxy.run`; pooling reuse gate lifted). **Remaining** ⬜: LB algos (least-conn/weighted/consistent-hash — round-robin done), health-checked backend pool, PID-controller autoscaling (spawn/kill backends via R1) | ⭐user P1 | ◑ | `167_proxy_pool` + live | proxy `db492a9`/`423f666`; multi-core via N1 |
