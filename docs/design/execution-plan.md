...
| **I1** | **Nova reverse proxy + load balancer + PID autoscaler** (⭐"most important", user) — L4/L7 proxy on the async runtime + TCP server + **HAProxy-style per-reactor lock-free conn pool**, **share-nothing MULTI-CORE via N1** (per-reactor SO_REUSEPORT accept fan-out; `Proxy.run`; pooling reuse gate lifted). **LB algos DONE** (`LbStrategy`: round-robin/weighted/least-conn/consistent-hash, all lock-free per-reactor; `178`). **Health-checked backend pool DONE** (active TCP-connect / HTTP-GET probes, rise/fall drain hysteresis, drained backends excluded from every strategy; sweep coroutine on reactor 0; live drain-proven; `179`). **Remaining** ⬜: PID-controller autoscaling (spawn/kill backends via R1) | ⭐user P1 | ◑ | `167_proxy_pool` + `178` + `179` + live | proxy `db492a9`/`423f666`; multi-core via N1; LB `ab07a2f`; health `b70d371` |
