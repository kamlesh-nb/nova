...
- `puts_fn`, `printf_fn`, `nova_log_string_fn`, `nova_log_info_fn`, `nova_log_debug_fn`, `nova_log_err_fn`, `log_fn` -- cached runtime/libc function values (declared lazily).
...
- **`fn getOrCreateStringLiteral(self, str) !LLVMValueRef`** (pub). Interns a string literal to an internal-linkage global laid out as `{ i32 refcount, i32 len, [N x i8] chars }`, then returns a `ptrtoint` of the chars field into `val_type` (so a Nova string is the address of the char data, with the header 8 bytes before it). The refcount initialiser is **-1000000000** (a NEGATIVE sentinel), which makes `nova_retain`/`nova_release` full no-ops (they early-return on `*rc < 0`) so the shared global is immortal and never drifts to a double-free. On a cache hit it still rebuilds the GEP+ptrtoint in the current block. Allocates a duped key stored in `string_globals` (owned by the map).

- **`fn getOrCreateDecimalLiteral(self, digits) !LLVMValueRef`** (pub). Interns a decimal literal (`0m`, `2m`, ...) to a lazily-initialised `val_type` global. Emits a runtime check `if (cache == 0) { parse once via nova_decimal_from_string; pin its header refcount to -1000000000; store into cache }` then a phi of cached-or-parsed. Turns a per-evaluation heap-alloc+parse into a load plus a predicted branch. The pinned negative refcount makes sharing it across owners safe. Builds three basic blocks (`dec_init`, `dec_cont`) and a phi.
...
- **`fn compileAlloc(self, size) !LLVMValueRef`** (pub). Lazily declares and calls `nova_bytes_alloc(val_type) -> val_type` (the ARC-headed heap allocator). Returns the client pointer as a `val_type` word. The workhorse allocator for strings, structs, enum boxes, trait objects.

- **`fn compileAllocArray(self, size) !LLVMValueRef`** (pub). Same but calls `nova_array_alloc(val_type) -> ptr` returning a real `ptr` (not an int), so fixed-array construction keeps pointer provenance and LLVM can vectorize/hoist the access loops.

- **`fn compileAllocPersistent(self, size) !LLVMValueRef`** (pub). Calls `nova_bytes_alloc_persistent` for allocations that outlive a request arena.
...
- **`fn buildValoptBox(self, value) !LLVMValueRef`** (pub). Lazily declares and calls `nova_valopt_box(val_type) -> val_type` -- heap-boxes a raw value so a value-optional slot can distinguish present-holding-0 from absent (0).

- **`fn buildValoptUnbox(self, box) !LLVMValueRef`** (pub). The inverse: `nova_valopt_unbox`. Peels one box level, yielding the raw payload word.

- **`fn buildAnyBox(self, payload, dtor) !LLVMValueRef`** (pub). Calls `nova_any_box(payload, dtor) -> val_type`, building a `{payload, dtor}` owning carrier so an `any` slot records how to release its payload.

- **`fn buildAnyUnbox(self, box) !LLVMValueRef`** (pub). Calls `nova_any_unbox` to recover the payload word.
...
- **`fn compileFree(self, ptr) !LLVMValueRef`** (pub). Lazily declares and calls `nova_bytes_free(val_type) -> void`; returns a constant 0 word.

- **`fn generateWasmMemoryFunctions(self) !void`** (pub). Emits the whole WASM bump-allocator by hand: `nova_bytes_free` (frees only arena-region or zero pointers, else pushes onto the free list), `nova_bytes_alloc` (seeds the heap from `__heap_base` on wasm, writes a 4-byte size header, bumps `heap_ptr` with 8-byte alignment), `nova_bytes_alloc_persistent` (a free-list first-fit walk with bump fallback into the persistent region), and stub `nova_retain`/`nova_release` (no-op ret void, since wasm uses the arena model). Builds many basic blocks with its own temporary builders. Registers all into `func_map`.
...
- **`fn compileCoverageIncrement(self, block_id) !void`** (pub). Emits `__nova_cov_counters[block_id] += 1` against the external coverage-counter global for coverage-instrumented builds.
...
- **String and decimal literals carry a NEGATIVE (-1000000000) refcount sentinel** so `nova_retain`/`nova_release` are no-ops on them (they early-return on `*rc < 0`). This replaced a `+1e8` count that drifted to a double-free after ~1e8 reuses. Never re-introduce a positive "large" count for immortal shared objects.
