...
This is the design document for the Kyte compiler. It is written top down: first the whole architecture and
...
The compiler is written in Zig 0.16. It turns Kyte source into LLVM IR, then a native object, then a linked
...
Kyte is a statically typed, ES6/TypeScript-flavoured language. The compiler is a classic multi-stage
...
source text
  ...
  sema's typed IR
  -> optimiser        -> HIR -> MIR -> LIR, run passes, then either emit LLVM directly (KYTE_OPT_EMIT)
                         or just report a shadow diff (KYTE_OPT). Per function, the emit path falls back
...
- **ARC (automatic reference counting).** Memory is not garbage collected. Every heap object has an 8-byte
  header (refcount at ptr-8, length at ptr-4). `kyte_retain`/`kyte_release(ptr, dtor)` adjust the count;
...
The `formatter.zig` (`kyte fmt`) sits beside the frontend and is a separate consumer of the AST; it is
covered with Part 1.
...
tree. It is responsible only for *shape*: is this valid Kyte grammar, and what tree does it describe. It makes
...
**Purpose.** `kyte fmt`. A second, independent consumer of the AST that walks the tree and re-emits canonical
...
Kyte's inference and coercion rules. It handles the hard cases: closures whose parameter types must be pinned
...
plumbing. It also carries intrinsics that never call a real Kyte function (SIMD, `mem.*`/`bytes.*` typed
...
**Purpose.** `arc.zig` is the memory model made concrete: the 8-byte header, `kyte_retain`/`kyte_release`, and
...
optimisation passes, then either reports a diff against the AST backend (the shadow, `KYTE_OPT`) or emits LLVM
directly (the emit path, `KYTE_OPT_EMIT`). Both are off by default. The emit path is a strict per-function
...
package/scaffold commands and the `~/.kyte` install layout that produced binaries depend on.
...
- **`pipeline.zig`** - the compile pipeline: a cycle-guarded import walk (`loadProgram`) that merges the whole
  ...
  emit Kyte source and re-parse it), the LLVM object emission, and the link step (in-process LLD, `clang++`,
  or cross via bundled `zig c++`), plus cross-compilation and the `~/.kyte` layout.
- **`builder.zig`** and **`tester.zig`** - `kyte build` (project.json, profiles, a content-hashed `.o` cache)
  and `kyte test` (@test discovery and run). Both set the optimiser emit flags identically.
- **`format.zig`**, **`packages.zig`**, **`scaffold.zig`**, **`templates.zig`** - `kyte fmt`, dependency
  fetch into `~/.kyte/cache`, and `kyte init` app/web scaffolding.
...
```
zig build                  # build kyte, install to ~/.kyte
KYTE_ASAN=1 zig build      # also build the sanitized runtime for the --asan gate
kyte <file> -o out         # compile one file
kyte test <file>           # run @test (skips main())
...
Debug: `KYTE_DUMP_MERGED=1` dumps merged IR; `KYTE_SEMA_SHADOW=1` diffs the type engines;
`KYTE_OPT_EMIT_VERBOSE=1` logs which functions the emit path took or rejected.
