...
This part of the low-level design covers the "outer shell" of the Nova compiler: the process entry
point, the argument dispatch layer, the shared compile/link pipeline that turns Nova source into a
linked binary, and the individual command drivers (`build`, `test`, `fmt`, `get`/restore, `init`/`add`)
that sit on top of it. A single invocation `nova <cmd> ...` flows like this: the OS calls
...
`packages` (get), or `builder` (build and the bare `nova <file>` compile form). Every compiling command
...
down by `zig build` under `~/.nova` (the `HOME`/`USERPROFILE` directory), specifically `~/.nova/std`
(the standard library sources read as a fallback), `~/.nova/lib` (the C++ runtime archives such as
`libnovacore.a` and `novacore_asan`), `~/.nova/cache` (fetched package git repos), and `~/.nova/deps`
(vendored native libraries such as webview). A `.nova` path under `src/std/` is first looked for in the
current working directory, then in this shared `~/.nova/std` copy, which is why a compiler installed
...
- Reads `build_options.nova_version`, `build_options.nova_abi_version` (for the `version` command) and
...
- **`fn run(init: std.process.Init) !void`** (pub; the dispatcher)
  ...
  - Fall-through: everything else (including `build` and a bare `nova <file>`) goes to
    `builder.cmdBuild`.
...
- **`TargetInfo = struct { os, arch: []const u8, ptr_size: u8, is_posix: bool }`**: compile-target facts
  ...
  "windows" | "wasm"; `arch` is "aarch64" | "x86_64" | "wasm32". Exposed to Nova source as the
...
**Module-level state / constants:**
- `extern fn nova_lld_link_macho(...)` and `extern fn nova_lld_link_wasm(...)`: the in-process LLVM LLD
...
- **`fn appendFfiLib(args: *ArrayList, allocator, shared_nova, io, lib) !void`** (pub)
  - Appends the right link tokens for one FFI library. Special-cases `"webview"`: links
    `<shared_nova>/deps/webview/build/libwebview.a` (erroring `LinkFailed` if it is not built) plus the
...
- **`fn linkNativeInProcessMacho(allocator, environ, io, objs, output_path, shared_nova, ffi_libs)
  !void`** (pub)
  - The in-process macOS link via `ld64.lld` (no `clang++` process spawn). Builds an argv:
    ...
    `-dead_strip`, the objects, `-L<shared_nova>/lib -lnovacore -L/opt/homebrew/lib`, the (no-op) wolfSSL
    link, each FFI lib, and `-o <output>`. Duplicates each arg as a null-terminated C string and calls
    `nova_lld_link_macho`. Non-zero return → `error.LinkFailed`.
...
- **`fn crossLinkViaZig(allocator, environ, io, llvm_triple, objs, output_path, shared_nova, is_release)
  !bool`** (pub)
  - The cross-compile link. Resolves the target via `mapCrossTarget` (returns false if none). Ensures the
    C++ runtime object for that target exists at `<shared_nova>/lib/novacore_<zigtarget>.o`; if missing it
    one-time cross-compiles `<shared_nova>/src/runtime/runtime.cpp` with `zig c++ -target <t> -std=c++20
    -O2 -DNOVA_DROP_ARENA -c` (spawned process; non-zero → `LinkFailed`). Then spawns `zig c++ -target <t>`
  ...
  - Side effects: spawns up to two `zig c++` processes; reads/writes the cached runtime object under
    `~/.nova/lib`. `environ` is unused (Boost include retired in M4).
...
- **`fn linkWasmInProcess(allocator, obj_path, output_path) !void`** (pub)
  - The in-process wasm link via `wasm-ld`: `--no-entry --export-all --export-memory --allow-undefined
    ...
    `nova_lld_link_wasm`. Non-zero → `LinkFailed`.

- **`fn appendRuntimeLink(args: *ArrayList, allocator, shared_nova, lib_name) !void`** (pub)
  - Appends the Nova C++ runtime to a `clang++` link line. On Windows, links the COFF object
    `<shared_nova>/lib/<lib_name>.o` directly (MSVC's link.exe cannot read the GNU archive llvm-ar writes),
    adds `-rtlib=compiler-rt` (for the 128-bit `__udivti3`/`__umodti3` helpers) and `-lws2_32 -lmswsock
    -lbcrypt`. Elsewhere, appends `-L<shared_nova>/lib -l<lib_name> -L/opt/homebrew/lib`.
  - `lib_name` is `novacore`, `novacore_asan`, or `novacore_tsan` depending on the sanitizer.

- **`fn appendWolfsslLink(args, allocator, shared_nova, io) !void`** (pub)
  - A deliberate no-op. wolfSSL was retired in M13 (TLS is pure Nova). Kept so the three link sites need no
    change. All parameters discarded.

- **`fn configureValueStructs(allocator, environ: anytype) void`** (pub)
  - Configures the M-1 value-type-struct codegen gate from the environment. `NOVA_VALUE_STRUCTS_OFF` set →
    return early (all-reference model, the escape hatch). `NOVA_VALUE_TYPES=A,B,C` → tokenise the list,
...
- **`fn getSharedAssetPath(allocator, init, relative_path) ![]const u8`** (pub)
  - Resolves an asset by preferring the CWD-relative path if it exists, else falling back to
    `<home>/.nova/<relative_path>`. Returns an owned path.

- **`fn hostOs() []const u8`** / **`fn hostArch() []const u8`** (pub): map `builtin.target` to Nova's
...
- **`fn genPlatformSource(allocator, t: TargetInfo) ![]const u8`** (pub)
  - Generates the source of the compiler-synthesized `platform` module (never a file on disk): the `os`,
    `arch`, `pointerSize`, and the `isDarwin`/`isLinux`/`isWindows`/`isWasm`/`isPosix` booleans as Nova
    `pub const`s. Returned owned string.

- **`fn suffixedFileExists(cand, allocator, io, home) bool`** (pub)
  - True if `cand` (a `.nova` path) exists in the CWD, or, for a `src/std/`-prefixed candidate, under the
    installed `<home>/.nova/std/<sub>` fallback. Two `Io.Dir.access` probes.

- **`fn targetVariantPath(path, os_tag, arch, is_posix, allocator, io, home) ?[]const u8`** (pub)
  - Platform-axis variant selection. Given a resolved `dir/name.nova`, picks the most-specific existing
    target variant (first existing wins): `dir/<os>/<arch>/name.nova`, `dir/<os>/name.nova`, then (posix
    only) `dir/posix/<arch>/name.nova`, `dir/posix/name.nova`, then the legacy suffix
    `dir/name_<os>.nova`. Returns null when none exists (caller reads the flat base path). Special-cases
    the event-loop module: `src/std/net/eventloop` maps to `src/std/net/ev/<mechanism>.nova` where
...
- **`fn existingSource(nova_candidate, allocator, io) ?[]const u8`** (pub)
  - Given a freshly allocated `.nova` candidate, returns whichever of `<path>.nova` or its `.nsx` sibling
    exists (as an owned path); `.nova` wins on a tie. Frees the candidate and returns null if neither
...
- **`fn resolveImportPath(base_path, module_name, allocator, io, home) ![]const u8`** (pub)
  - The central import resolver. Order: `platform` → the synthetic `src/std/platform.nova`; `std/<x>` →
    `src/std/<x>.nova`; a long hard-coded `std_modules` allow-list of bare std module names →
    `src/std/<name>.nova`; a handful of bare aliases (`list`/`map`/`set`/`string_builder`/`deque`/`heap`/
    `ordered_map` → `src/std/collections/...`, `db` → `src/std/data/db.nova`, `pool` →
    `src/std/data/sql/pool.nova`). Then importer-relative resolution, walking up from the importer's own
    directory and trying `<dir>/src/<module>.nova` and `<dir>/<module>.nova` at each level (this WINS over
    any global package match, which is what lets driver packages drop per-driver prefixes on their internal
    modules). Then `resolveFromLocalPackages`, then `src/<module>.nova`, then `resolveFromPackageCache`.
    Final fallback: `<dir>/<module>.nova` (or `<module>.nova` at root). Returns an owned path.

- **`fn resolveFromPackageCache(module_name, allocator, io, home) ?[]const u8`** (pub)
  - Scans `<home>/.nova/cache/*/` (the git-fetched package cache) for `<pkg>/src/<module>.nova` or
    `<pkg>/<module>.nova`. Returns the first existing owned path, else null. Opens and iterates the cache
    directory.

- **`fn scanPackageRoot(root, module_name, allocator, io) ?[]const u8`** (pub)
  - Scans one `packages/` root: first `<root>/nova-<module>/src/<module>.nova` (the package's own top
    module), then any `<root>/<pkg>/src/<module>.nova` (a flat module inside any package). Returns an owned
    path or null.
...
- **`fn generateSerdeBinders(allocator, declarations: *ArrayList, is_wasm) !void`** (pub; codegen prelude)
  - The big one. For every `@serializable` struct it generates Nova source for a family of functions and
...
- **`fn generateRuntimeMediator(allocator, declarations: *ArrayList, is_wasm) !void`** (pub; codegen
  prelude)
  - The runtime mediator glue (pairs with `web/rmediator.nova`). Only requests that `impl Message` opt in
...
- **`fn loadProgram(allocator, init, file_path, visited, visiting, merged, declarations, is_wasm,
  ...
  - The recursive source loader and DFS import resolver. Guards against cycles (`visiting` set →
    ...
    the actual bytes to read via `targetVariantPath` (so a `foo_<os>.nova` or `os/<os>/...` variant is read
    while module identity stays `file_path`); the synthetic `src/std/platform.nova` is generated in-memory
    via `genPlatformSource`. For `src/std/` paths, reads the CWD copy and falls back to
    `<home>/.nova/std/<sub>` on `FileNotFound`. Parses the source, and for every `import_decl` (skipping
    ...
    `merged`.
  - Side effects: reads source files (CWD and `~/.nova/std`); populates `file_sources` (keys and the
...
- **`fn findNovaFiles(allocator, io, root_dir, sub_path, list: *ArrayList) !void`** (pub)
  - Recursively collects `.nova` and `.nsx` files under `root_dir/sub_path` into `list`, skipping dotfiles,
    `zig-cache`, `zig-out`, `lang`, and the generated `merged.nova`. Each collected path is owned. Used by
...
- **`fn linkLibsStamp(allocator, init) u64`** (pub): XOR-folds the mtimes of the shared runtime archives
  (currently `~/.nova/lib/libnovacore.a`) into a `u64`, so a runtime rebuild invalidates the `.o` cache.

- **`fn sourcesHash(file_sources, is_release, asan, link_stamp) u64`** (pub)
  - The content hash keying the `nova build` incremental cache. Seeds with `CACHE_VERSION`, the
...
drives `nova test` over `conformance/cases/`.

---

## `src/builder.zig` (591 lines)

**Role in the pipeline:** The driver for `nova build [...]` and the bare `nova <file> [...]` compile form.
...
- **`fn compileProgram(allocator, init, file_path, target, output_path, is_release, target_triple_opt,
  ...
  - **Sanitizer gating:** `asan = !is_wasm and (NOVA_ASAN != "0" if set, else !is_release)`. So debug
    native builds default to ASAN on, `--release` off, wasm never. `NOVA_ASAN_CODEGEN` (implies asan)
    additionally instruments Nova-generated code (`codegen_arc.asan_codegen_enabled`).
  - **Load:** loads `src/std/collections/string_builder.nova` first (warn-only on failure), then the user
    `file_path`, via `pipeline.loadProgram`. `NOVA_DUMP_MERGED` writes the merged IR to `merged.nova`.
  ...
  - **Frontend/sema:** builds `ast.Program` from the declaration list; runs `sema_alpha.run`,
    ...
    (`NOVA_SEMA_SHADOW`, `NOVA_TID_CENSUS`), ARC elision default-on unless `NOVA_ARC_ELIDE_OFF`, then
  ...
  - **Escape/optimiser flags:** `NOVA_ESCAPE_REPORT` runs the report-only P7 escape gauge. Then the emit
    flags: `lir_emit.emit_enabled = (NOVA_OPT_EMIT != null)`, `lir_emit.emit_verbose =
    (NOVA_OPT_EMIT_VERBOSE != null)`, and `optimiser/mir.zig.type_store = &owned_sema.store` (so the
    optimiser's width-honest constfold can resolve TypeId widths). `NOVA_OPT` runs the shadow lowering
  ...
  - **Codegen + link (native):** picks the object path (`<build_obj_dir>/<basename>.o` in build mode, else
    `<output>.o`). T6 per-file split is default-on in build mode unless `NOVA_T6_NOSPLIT`, opt-in via
    `NOVA_T6_SPLIT` otherwise; `split_objs` collects the pieces. Calls `llvm_codegen.compile`, then reports
    ...
    `-pthread -I.`, the shared `-I<home>/.nova` include, the objects, the runtime link
    (`appendRuntimeLink` with `novacore_asan` or `novacore`), the FFI libs, and `-o <output>`.
    ...
    - Object cleanup: deletes the object unless `NOVA_KEEP_OBJ` (and never in build mode). In build mode,
  ...
  - Side effects: reads sources, writes objects and the final binary, spawns `clang`/`clang++`/`zig c++`,
    writes the build-hash and (optionally) `merged.nova`. Allocations are arena-scoped by the caller.
  - Gotchas: env access is always `init.environ_map.get`. The shared `~/.nova` install supplies the
...
- **`fn cmdBuild(allocator, init, args) !void`** (pub; the entry the CLI dispatches to)
  - First calls `packages.ensureDependencies` (auto-clone any missing `project.json` dependency so a fresh
    clone builds without a manual `nova get`; silent when there is no `project.json`).
  - Parses args. Two arg grammars: `nova build ...` (flags `--target <wasm|native|cross>`, `--file`, `-o`,
    `--release`/`-r`, `--debug`/`-d`, `--watch`/`-w`) and the bare `nova <file> ...` form (positional file
    then `--wasm`/`--native`, `--release`/`--debug`, `--watch`, `--target`/`-t <cross>`, `-o`).
  - **Project layout (build_mode):** default `file_path = src/main.nova`; project name from `project.json`
...
**Role in the pipeline:** The driver for `nova test`. It discovers `@test` functions across the target
...
`__nova_test` binary. It sets the same emit/sanitizer flags as the build path so the conformance gate run
via `nova test` genuinely exercises them.
...
- **`fn generateTestHarness(test_fn_names, allocator) ![]const u8`** (private)
  - Generates the Nova source of a `main()` that, for each test: calls `nova_test_reset()` /
    `nova_test_begin(name)`, invokes the test, and prints `PASS`/`FAIL` (with the failure message from
    `nova_test_fail_message()`), tallying passed/failed/total. After all tests it prints the summary,
    then `nova_exit(1)` if `nova_arc_audit_report() > 0` (ARC leaks) or any test failed. Returned owned
    source.
...
- **`fn cmdTest(allocator, init, args) !void`** (pub; the entry the CLI dispatches to)
  ...
  - **Discovery:** if no file is given, `pipeline.findNovaFiles` scans the CWD for all `.nova`/`.nsx`
  ...
  - **Emit flags:** sets `lir_emit.emit_enabled`/`emit_verbose` from `NOVA_OPT_EMIT`/`NOVA_OPT_EMIT_VERBOSE`
  ...
  - **Codegen + link:** always emits to the hardcoded output `__nova_test` (object `__nova_test.o`). T6
    split is opt-in via `NOVA_T6_SPLIT`. `asan` is opt-in here (`NOVA_ASAN != "0"`, default off, unlike the
    build path); `tsan` via `NOVA_TSAN`. Assembles a `clang++` line (`-std=c++20 -g -O0 -pthread`,
    dead-strip + PIE flags, `-I.`, the `-I<home>/.nova` include, the sanitizer flags, the objects, the
    runtime link picking `novacore_asan`/`novacore_tsan`/`novacore`, the FFI libs, `-o __nova_test`).
    Spawns `clang++`; non-zero → `LinkerFailed`. Deletes the object.
  - **Run:** spawns `./__nova_test`. A non-zero exit (or abnormal termination) prints a failure line and
    the whole command `std.process.exit(1)`.
  - Gotchas: `nova test` skips the project's `main()` and runs the imported `@test`s (a measurement trap
    noted in the memory: use `NOVA_ARC_DUMP`/`NOVA_ARC_AUDIT` to see survivors). The hardcoded
    `__nova_test` output is why concurrent `nova test` runs clobber each other, which the `conformance
    run.sh -j` mode works around by running each case in its own temp directory.

**Cross-references:** the harness relies on the runtime's `nova_test_*` and `nova_arc_audit_report`
...
**Role in the pipeline:** The driver for `nova fmt`. It pretty-prints Nova source via
...
`lexer`, `parser`, `formatter`, and `pipeline.findNovaFiles` are exercised).
...
- **`fn formatFile(allocator, init, file_path) !void`** (private)
  - Reads the file, parses it (`is_wasm=false`), runs `formatter.Formatter.formatProgram`. If the
    ...
    under `NOVA_FMT_DEBUG`, the first differing token pair) rather than risk altering code. Otherwise it
...
- **`fn cmdFmt(allocator, init, args) !void`** (pub; the entry the CLI dispatches to)
  - With a path argument (`args.len >= 3`), formats that one file (errors are printed, not fatal). With no
    path, uses `pipeline.findNovaFiles` to format every `.nova`/`.nsx` file under the CWD, counting
    successes and printing a summary.

**Cross-references:** relies on `frontend/lexer.zig`, `frontend/parser.zig`, `frontend/formatter.zig`;
uses `pipeline.findNovaFiles` for the format-all path. No codegen or optimiser interaction.
...
`~/.nova/cache`, auto-fetches missing `project.json` dependencies before a build/test, and implements
`nova get <url>` (add a dependency and clone it) and the bare `nova get` restore (clone every declared
dependency).
...
- **`fn cloneIntoCache(allocator, init, git_url) !bool`** (private)
  - Clones `git_url` into `<home>/.nova/cache/<repo>`. Returns false (already cached) if the target dir
...
- **`fn ensureDependencies(allocator, init) !void`** (pub): the pre-build/pre-test auto-fetch. Reads
  ...
  dependency not already in `~/.nova/cache/<repo>` clones it (via `cloneIntoCache`). Deliberately quiet:
...
- **`fn cmdRestore(allocator, init) !void`** (pub): the bare `nova get` path. Reads and parses
...
**Role in the pipeline:** Project scaffolding for `nova init <console|web|desktop> --name <name>` and
`nova add feature <name>`. It writes template files (from `templates.zig`) into a new project directory
...
- **`fn scaffoldWeb(allocator, io, project) !void`** (private): writes the whole web-app template tree: the
  composition-root `src/main.nova`, the vertical-slice Features (CreateProduct command/response/validator/
...
- **`fn scaffoldDesktop(allocator, io, project) !void`** (private): writes just
  `src/main.nova` from `templates.desktop_main_sample` (a webview window binding a Nova handler to a JS
  call).
...
- **`fn cmdInit(allocator, init, args) !void`** (pub; the entry the CLI dispatches to)
  - Validates the template type (`console`/`web`/`desktop`; `app` is accepted with a deprecation note and
    ...
    `src/main.nova` and `tests/main_test.nova`; for `web`, calls `scaffoldWeb`; for `desktop`, calls
...
- **`fn cmdAddFeature(allocator, init, name) !void`** (pub; the entry `cli.run` dispatches `add feature`
  to): creates `features/<name>/` with stub `model.nova`, `service.nova`, `view.nova`, and
  `<name>.nova`, then (if `project.json` exists) splices the feature name into a `"features": [ ... ]`
...
- `console_main_sample`, `console_test_sample`: the console app's `main.nova` and a sample `@test`.
...
- `desktop_main_sample`: the webview desktop `main.nova`.
