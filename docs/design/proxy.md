# Building a High-Performance L4 Proxy in Nova — Control/Data-Plane Split

**Status:** design note (not scheduled — captured for reference).
**Date:** 2026-07-19.
**Question it answers:** *Can Nova build an HAProxy-class proxy with load-balancing + PID auto-scaling,
...
**Yes — provided the L4 data path lives in the C++ runtime, not in Nova.** This is not a workaround; it
...
- **Data plane (bytes) → C++ runtime.** Pure byte-forwarding at wire speed, with kernel zero-copy. No
  ARC, no Nova heap object ever touches a payload byte.
- **Control plane (logic) → Nova.** Backend selection policy, health, config, and the PID auto-scaler —
...
| Concern (if L4 were in Nova) | Resolved by the split |
|---|---|
...
> **The payload path is mechanism (C++). The routing decision is policy (Nova). They meet at a
> lock-free snapshot, never on the per-byte path.**

Nova computes *what the policy is*. The runtime *executes* it. A byte being proxied crosses zero
...
```
            ...
            │        └──────────────┬──────────────────────┴────────────────────┘            │
            │                       ▼                                                         │
            │              build immutable SNAPSHOT  ──►  nova_l4_policy_publish()            │
            └───────────────────────────────────────────────│─────────────────────────────┘
                                                             │  atomic ptr swap (RCU)
            ┌────────────────────────────────────────────────▼────────────────────────────┐
...
io_context, multi-threaded pool, and per-socket strands). Everything above it is ordinary Nova.

---

## 4. Data plane — a C++ runtime primitive

The L4 forwarder is added to the runtime in the same style as the existing `nova_socket_*` primitives.
It is the only new C++ of substance.

### Responsibilities
- Accept connections on the io_context (async, across the thread pool).
- Read the current policy **snapshot** locklessly and select a backend (or invoke a per-connection Nova
  callback — see §6).
...
always C++; FFI is only for *user* Nova code binding external C.

### Proposed runtime ABI (called from Nova by name, like every `nova_*` primitive)
```c
// Listener / server lifecycle.
long long nova_l4_listen(const char *bind_addr, int port);      // -> listener handle, or -1
void      nova_l4_serve(long long listener, long long policy);   // async accept loop on the io_context

// Policy handle: holds an atomically-swappable pointer to the current snapshot.
long long nova_l4_policy_new(void);                              // -> policy handle
void      nova_l4_policy_publish(long long policy,               // atomic swap-in a new snapshot;
                                 long long snapshot_ptr, int n);  //   old one retired after a grace period

// Telemetry the control plane reads (per backend index).
long long nova_l4_active_conns(long long policy, int backend_idx);
long long nova_l4_total_bytes(long long policy, int backend_idx);
```
...
It must **never** allocate or refcount a Nova heap object per connection or per read. ARC is for the
control plane's data structures only.

---

## 5. Control plane — Nova

Low-frequency, expressive, safe. This is the part that is genuinely "written in Nova."

### 5.1 Backend registry & LB policy
A plain Nova structure the control plane owns and mutates:
```
struct Backend { pub addr: string; pub port: int; pub weight: int; pub healthy: bool; }
```
Load-balancing algorithms are pure compute (structs + maps + math) — all straightforward in Nova:
- **Round-robin** / **weighted round-robin**
- **Least-connections** (reads `nova_l4_active_conns`)
...
### 5.2 Health checks
Async probes (TCP connect / HTTP GET) on the existing Asio runtime — Nova `async fn` + timers. Mark
...
### 5.3 PID auto-scaler
A textbook discrete PID controller — a handful of floats and a timer loop. Nova has `f64`/`decimal`,
...
- **Process-level:** spin backends up/down via `nova_process_spawn` (exists).
- **Orchestrator:** call a k8s / cloud API — Nova has HTTP client + JSON + TLS (wolfSSL).

Anti-windup, derivative filtering, and setpoint choice are algorithmic and language-agnostic.

### 5.4 Config
Parse + hot-reload config in Nova (JSON/YAML both present in stdlib). A reload rebuilds the registry and
...
Nova builds an immutable array of `{addr, port, weight, healthy, rank}` in a raw `bytes` buffer (not ARC
objects), then hands its pointer to `nova_l4_policy_publish`.

### Publish = atomic pointer swap (RCU-style)
```
    Nova: build new snapshot ──► nova_l4_policy_publish(policy, ptr, n)
...
1. **C++ selects from the snapshot (recommended for throughput).** Nova ranks backends into the
   snapshot; C++ does the final pick (round-robin index, P2C, hash) from the plain array. **Zero Nova
   calls on the connection path.**
2. **Nova callback per connection.** C++ calls a Nova function returning a backend index. One managed
   call per *connection* (not per byte) — thousands/sec is negligible — if you want the full algorithm
   in Nova. Costs a boundary crossing per connection; fine for most workloads, avoid for extreme conn
   rates.
...
- **Native codegen (LLVM), no GC** — deterministic dealloc, no pause-induced tail latency. Nova is in
  the C/C++/Rust tier, not Go/Java.
- **Hot path is C++ + Asio + kernel zero-copy** — the same substrate high-end C++ proxies use. Nova
...
native language + async runtime + manual memory where it counts. Nova's model is analogous.
...
|---|---|---|
| **Binary-safe socket send** | `nova_socket_send` uses `strlen` → truncates at the first null byte. A proxy moves arbitrary bytes. | Add `nova_socket_send_n(fd, ptr, len)`. ~½ day. **Hard prerequisite.** |
| **Async socket path exposed** | The data plane needs async accept/read/write on the io_context, not the blocking `nova_socket_*`. Confirm/expose the Asio path to the L4 primitive. | Verify; likely wiring, not new architecture. |
| **`nova_l4_*` runtime primitive** | The splice loop + policy handle (§4). | New runtime C++, moderate. |
...
- **Terminate + inspect in C++, route via Nova policy** — parse headers in the runtime, cross to Nova
  only for the routing decision (per request, not per byte). Keeps most of the cost in C++.
- **Hand the request to Nova** — full flexibility (write handlers in Nova), pay the managed cost per
  request. Fine for moderate rates; this is where Nova's web stack (`web/`) already lives.

How much sits in C++ vs Nova is a **per-feature tradeoff**, decided by that feature's request rate and
how much logic it needs. L4 = all-mechanism-in-C++; a rich L7 gateway = more-in-Nova.

---

## 10. Honest caveats

- **The L4 data plane is C++, not Nova.** "Written in Nova" applies to the control plane. Normal for
  proxies (Envoy's data plane is C++), but it reframes the story: **Nova orchestrates a C++ data
  plane** for L4.
- **Maturity is the real risk.** Nova's ARC foundation was stabilized recently; the compiler is
  Alpha/Beta. A proxy — long-running, million-connection, adversarial input — is exactly the workload
  that surfaces latent compiler/runtime bugs. The control plane is where Nova code runs continuously;
...
2. **Data plane v0:** `nova_l4_listen`/`nova_l4_serve` with a *userspace-copy* splice loop (portable),
   static single backend. Prove end-to-end forwarding.
3. **Policy seam:** `nova_l4_policy_new`/`publish` + the RCU snapshot; C++-side round-robin selection.
4. **Control plane:** Nova backend registry + LB algorithms (RR → least-conn → P2C) building snapshots.
...
- Nova substrate: `src/runtime/concurrency.cpp` (Asio io_context, strands), `src/std/net/tcp/*`
