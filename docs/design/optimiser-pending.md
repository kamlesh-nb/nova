...
| B5 | Value-struct params / returns | P2 | FALLBACK | Safe AST fallback. Full emit needs the by-value struct ABI (inline bytes, possibly >8 bytes, split/byref) + inline field access -- a real ABI decision. Low value in measured builds. |
