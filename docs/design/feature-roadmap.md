# Nova — Feature Roadmap (beyond the F1–F5 foundation)
...
| **`main(args)`** | Real C `main(argc, argv)` + `nova_set_args`/`nova_arg_count`/`nova_arg_at` runtime shim, `env.args(): List<string>`, optional `main` args param. | ✅ **DONE** — commit `e33615a`, gate `54_process_args` (`./app a b 42` → argc=4 end-to-end). |
...
handlers, a **multi-threaded** context (background thread pool, `NOVA_THREADS`), **per-coroutine /
per-socket strands** for cross-thread safety, and a race-free waiter registry. `async`/`await` codegen
lowers to native coroutines (`is_async_native`). Nova `async fn` + `await` work end-to-end. This is
...
| **Networking maturity** | HTTP/1.1 + WebSocket via Boost.Beast, TLS via wolfSSL (partly present: `net/tls.nova`, `web/`). Harden on the Asio runtime; connection pooling / keep-alive. | runtime (done) | ~1–2 wk | Binary-safe I/O required (see the socket-send gap below). |
...
|---|---|---|---|
| **`RequestHandler<TReq,TResp>` trait** | in `web/mediator.nova`, `fn handle(self, req: TReq): TResp`. Replaces the `any`-based handler. Pipeline traits (`PipelineBehavior`, `PreProcessor`, `PostProcessor`, `ExceptionHandler`) stay. | generic traits | ~1–2 days |
...
| **`nova init app` template refresh** | rewrite the ASP.NET-style scaffold to the `mapGet<T>` + typed-handler API. | framework | ~1 wk (also Tier 6) |
...
Three production SQL wire protocols, implemented natively in Nova over the shared binary-socket I/O +
...
|---|---|---|---|
| **BSON decimal read accessor** | `findDecimal`/`docGetDecimal` (encode side + `entryDecimal` already exist). | bson.nova | ~½ day |
...
| **crypto** | SHA-256/HMAC/PBKDF2 (needed for SCRAM), plus general hashing/AEAD. `crypto/` is empty; `crypto.nova` minimal. | thin | ~1 wk |
| **HTTP client/server** | `web/client.nova` + `web/server.nova` exist; mature on the C++20 runtime, add connection pooling, keep-alive. | partial | ~1 wk |
| **collections** | `list`/`map`/`set`/`string_builder` exist; add ordered map, deque, priority queue as needed. | good | as-needed |
| **text/regex** | UTF-8 exists (`text/utf8.nova`); a regex engine is missing. | thin | ~1–2 wk |
...
"Can Nova build like Zig — without the user installing LLVM/clang/lld." Integration engineering, lower
...
- **Nova already compiles to WASM through LLVM** (AST → LLVM IR → LLVM `wasm32` → `.o`), with ~44
...
- **Zig's self-hosted WASM backend is not reusable** — it compiles Zig's IR (AIR), not Nova's AST. What
  Nova shares with Zig is the LLVM path (Zig's own default backend), which Nova is already on.
- **A Nova self-hosted WASM backend is worth it ONLY if the hard goal is dropping LLVM entirely** —
...
- ⇒ Today Nova needs **system LLVM *and* system clang** (the `clang++`/`clang -target wasm32` link
...
|---|---|---|---|
| **LLVM codegen (static)** | Nova → `.o`; static-link into `nova` | ✅ | **206 `libLLVM*.a`** at `/opt/homebrew/opt/llvm/lib` (LLVM 21.1.7) |
| **LLVM (dynamic)** | what `nova` links today | ✅ | `libLLVM.dylib` — the current runtime dep (`otool -L nova`) |
...
*only* for compiling the linker *into* `nova` (in-process `lld::wasm::link()`). Everything else needed
...
> the drop **cannot statically link `nova` via Zig's build.** (It stays reserved for the future in-process
> LLD that links `nova`'s *output* programs — a separate concern.)
>
...
> **native LLVM 22** tree → a **132 MB self-contained `nova`** whose only dylib deps are
...
> `nova` (linker compiled in via `lld::{macho,wasm,elf}::link()`) is the next step, no longer version-blocked.

**The path (Zig's distribution model).** Nova is already *built with* Zig 0.16.0, which bundles clang
...
| **Static LLVM** | Switch `linkSystemLibrary("LLVM")` → static-link the **206 `libLLVM*.a`** archives (already on disk) so `nova` carries codegen and needs no `libLLVM.dylib` at runtime. | Archives present; the `llvm-zig` `build.zig` needs to link the static components instead of the dylib (patch the dep or list the archives). | ~few days–1 wk |

**Sourcing decision to make explicit:** "self-contained" = either
- **(a) `nova` shells out to LLD/clang binaries or `zig`** as the link/cc driver — **everything needed is
...
- **(b) fully-standalone single `nova` binary** — static-link the 206 `libLLVM*.a` (present) **and**
...
truly single-file `nova` with the linker compiled in is a hard requirement.
...
| Piece | What | Effort |
|---|---|---|
| **Syntax + sema** | `extern("libcurl") fn curl_easy_init(): ptr;` — generalize the hardcoded `nova_*` extern path into a first-class extern decl. | ~2–3 days |
| **Type marshalling** | Nova ↔ C ABI: int/long/ptr/bool/double direct; `string` ↔ `char*` (borrow vs copy — Nova strings carry a length prefix); `extern struct` (repr-C) layout; array borrow-or-copy. **The substance.** | ~3–4 days |
...
| **Callbacks (Nova→C fn ptr)** | Optional/later: expose a non-capturing Nova fn as a C function pointer. | ~2 days |
...
| **`nova init app` template** | Improve the ASP.NET-style web-app scaffold (CLAUDE.md); wire it to the mediator/routing + a DB driver. | exists, rough | ~1 wk |
...
|---|---|---|---|---|
| 1 | **Conformance harness integrity** — `expect_fail` asserts a real diagnostic, not just exit≠0 (a segfault currently reads as "rejected"). Fix the silently-regressed `return_type_mismatch`. Turn `NOVA_ARC_AUDIT` on in the corpus. | Every negative result is unfalsifiable until this lands; regressions hide. | route §8.A | ~1–2 days |
...
| # | Item | Deps | Ref | Effort |
|---|---|---|---|---|
| 20 | **Toolchain self-sufficiency.** ✅ **Landed:** llvm-zig vendored (no GitHub fetch); `-Dstatic-llvm` static-links **native LLVM 22** (converted from the LTO drop via `llc`) → self-contained `nova`, no `libLLVM.dylib`; `-Dinprocess-lld` compiles the native `liblld*.a` into `nova` so it **links its own native executables via `lld::macho::link()` — no clang/ld shell-out** (gates green: FUNC 74/74, ARC 128/128 binaries run, SHADOW 128/128). The `--wasm` link is **also wired** to in-process `wasm-ld` and now works **end-to-end** — a nova program compiles + links to a valid wasm module (the #23 codegen blockers are fixed: test-harness externs, i32→i64 `val_type` for f64, `--allow-undefined` for host-import runtime symbols). Default stays dynamic (fast dev). **Remaining for full "deploy only nova":** replace the SDK-path lookup with **bundled macOS `.tbd` stubs** (currently uses the CLT/Xcode SDK); build the C++20 runtime with the bundled toolchain; ELF/Linux path (+musl+CRT). **Delivery invariant: users deploy only `nova`.** | ✅ native LLVM 22 + LLD (native+wasm links in-process) | Tier 4 | ~2 days left |
...
| 23 | **Tooling** — LSP maturity, `nova fmt`, package manager (finish F1-4), refresh `nova init app` to the mediator API. | flagship (#8) | Tier 6 | ongoing |
