...
| **FR-simd-L1** | Integer-vector SIMD (u8x16/u32x4/u64x2/i128 + ops + movemask) | P3 | not started (BLOCKER identified: the f64x4 slot/type machinery keys on a type-NAME string and encodes vectors as `prim{float,256}`; the three 128-bit integer vectors u8x16/u32x4/u64x2 all share 128 bits, so they cannot be distinguished by the current `prim{kind,bits,signed}` encoding. Needs a real vector TypeId variant (element-kind + lane-count) threaded through the type renderer, the slot picker in codegen/types.zig, and every sema switch, plus per-width ops + movemask in compileSimdCall. Genuine multi-day type-system work; the doc's hardware-acceleration tier. The software wins it would accelerate already shipped in FR-tls-1 + FR-deflate-4) |
| **FR-simd-L2** | Target crypto intrinsics (AES-NI, PCLMULQDQ, ARM pmull) + fallback | P3 | not started (depends on FR-simd-L1's vector types; target-gated behind a runtime CPUID/hwcap probe. The pure-Nova software fallback is already the shipping path and is fast after FR-tls-1 Shoup GHASH + AES key-schedule caching, so this is pure hardware headroom, not a correctness gap) |
