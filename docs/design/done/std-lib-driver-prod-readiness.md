# Kyte Standard Library + DB Driver Production Readiness

Status date: 2026-08-03
Scope: the Kyte standard library (`lang/src/std/`, 126 files, ~23k LOC) and the five
database driver packages (`packages/kyte-{postgres,mysql,mssql,mongodb,btreedb}`) plus the
shared DB seam (`data/db.ky`, `data/sql/pool.ky`, `data/orm.ky`).
...
|---|---|---|
| DB seam (`data/db.ky`) | Alpha | No transaction API, no error channel, lossy `DbValue`, client-side param interpolation. |
| Pool (`data/sql/pool.ky`) | Alpha | No max-open cap, no validation-on-borrow, no reconnect, not concurrency-safe. |
| Micro-ORM (`data/orm.ky`) | Alpha | Read side + thin `insert<T>`; no update/delete/upsert, no PK, scalar-only binding. |
...
- postgres: `decodeError` exists (`postgres.ky:262-274`) but `query`/`exec` never inspect the
  `'E'` frame; a failed query returns an empty ResultSet with `tag==""` (`postgres.ky:387-409`).
  Prepared paths only `break` on `'E'` (`:447,471,489`).
- mysql: `decodeErr` fully implemented + tested (`mysql.ky:499-510`) but live paths return only
  `tag="ERROR"` / `ExecResult(0,"ERROR")` (`:618,649,711,743`); connect-time auth ERR ignored.
- mssql: login failure logs to stderr and still returns a live-looking connection (`mssql.ky:877`).
- btreedb: `query`/`exec` loops do not handle frame type 69 (`btreedb.ky:353-363,371-377`).
- mongodb: `runCommand` never inspects `ok:0`/`errmsg`/`code` (`mongodb.ky:178-184`).
...
- No TLS at all: postgres (no SSLRequest), mysql (no CLIENT_SSL), mongodb, btreedb. `net/aio.ky`
  has no TLS entry point (`aio.ky:70-124`), so there is nothing to drop in. Cleartext passwords and
...
- TLS present but unverified: mssql tunnels the TLS handshake in TDS packets but forces
  `handshakeOk=true` and never validates the certificate (`mssql.ky:824-854`) -- unconditional
...
verification (`crypto/x509.ky`, `crypto/tls/truststore.ky`); the drivers simply do not use them.
...
(`db.ky:117-133`) -- no begin/commit/rollback, no savepoints, no isolation levels, no
rollback-on-drop. Callers must hand-issue `exec("BEGIN")` with no guarantee. On mssql this is worse:
the TDS transaction descriptor is hardcoded 0 and ENVCHANGE is ignored (`mssql.ky:179,199,577`),
...
handling only `$1..$9` (`postgres.ky:516-537`, `mysql.ky:782`, `btreedb.ky:101-124`). `$10+`
silently mis-substitute. The ORM `insert<T>` emits `$1..$N` into `exec` (`orm.ky:92-112`), so any
...
(`db.ky:51`, `postgres.ky:116,510`), so binary with NUL/quotes is unsafe on the simple path.
...
- postgres: Timestamp decoded with `parseI64` on an ISO string -> reads `2024` only
  (`postgres.ky:224`); bytea hex `\x` not decoded (`:231`); uuid/json/arrays fall through to raw text.
- mysql: text-protocol datetime does `parseI64("2024-01-01 10:00:00")` (`mysql.ky:277,97-98`),
  disagreeing with the (correct) binary path; binary drops microseconds (`:466`).
- mssql: FLOAT/FLT4/FLT8 decode to NULL (`mssql.ky:470-479`); DATE/TIME/DATETIME/DATETIME2/
...
- This ties back to C6 (the `DbValue` model has a Timestamp tag with no constructor/accessor,
  `db.ky:44-51`, and no date/time/uuid/json types).

### C6. `DbValue` model is lossy and incomplete
Single struct with `i:long/f:double/dec:decimal/s:string` (`db.ky:20-33`). Missing dedicated
date/time, UUID, JSON. NULL is a tag but accessors return 0/"" silently (no null-vs-zero on read,
`db.ky:35-37`). `asInt` truncates long->int. Blob is stored in the string slot.

### C7. Pool is not production-grade
`Pool` (`pool.ky:7-68`): no max-open/total cap (opens a new conn whenever idle is empty,
...
Grade: Alpha. Files: `data/db.ky` (133), `data/sql/pool.ky` (135), `data/orm.ky` (131).

- Connection trait: 6 async + 2 sync methods (`db.ky:117-133`). No transactions (C3), no batch,
  no streaming/cursor (results fully materialized, `db.ky:77-93`), no error channel (C1).
- DbValue: lossy + incomplete (C6).
- ORM read side: `queryAs<T>`/`queryOne<T>` via `RowSource impl ValueSource` (`orm.ky:10-68`),
  case-insensitive column index; scalar-only (`getChild` returns self, arrays/child -> empty,
  `orm.ky:37,43-47`). Write side: `insert<T>` exists (`orm.ky:92-112`) but emits `$N` (C4) and
...
(`postgres.ky:429-452`), exact decimal128.
Blockers: no TLS (C2); errors swallowed (C1); SCRAM server signature never verified though the helper
exists (`postgres.ky:572-573`, `crypto/scram.ky:44`); timestamp/bytea decode wrong (C5); MD5 auth
...
(desyncs on large rows, `mysql.ky:578-582`); multi-resultset unsupported (stored procs desync);
...
Blockers: TLS cert verification disabled unconditionally (C2, `mssql.ky:824-854`); FLOAT + all
...
defensive length guard against desync (`btreedb.ky:313`), buffered reader with compaction + dtor
freeing (past-leak fix), exact decimal128, documented reason for abrupt close (avoids nested-drive
deadlock).
Blockers: hardcoded `admin`/`kyte` credentials, DSN credentials ignored, no auth exchange
(`btreedb.ky:419`); `'E'` error frames swallowed (C1); prepared statements emulated via `$N` text
...
Blockers: `hello`/isMaster never sent -- `connect` opens the socket and returns (`mongodb.ky:237-247`);
...
(`compress/deflate.ky:482`, `crypto/tls/13/handshake.ky:50`, `net/tcp/client.ky:15`), plus two
...
- Crypto/TLS (Beta): 204 pub fns. TLS 1.3 client AND server (SNI, ALPN, HRR, resumption + 0-RTT +
  ...
  SHA-384 transcript flagged follow-on (`handshake.ky:50`). TLS 1.2 is client-only by DECISION: the
...
- IO/FS/OS (Beta): file + dir complete; process strong incl. `spawnIsolated` (namespaces/rootfs) and a
  ...
  glob, symlinks, mmap; `fs.ky` is only a Watcher.
...
2. TLS on the SQL data path (C2). Add a TLS `AsyncStream`/BIO variant to `net/aio.ky`, then wire
...
10. Async non-blocking DNS + IPv6 (stdlib gap 1): the current blocking IPv4-only getaddrinfo stalls the
    event loop (`net/eventedio.ky:70`).
...
- mssql: FLOAT (FLT4/8/N) decoded via kyte_ieee_le_to_str (was NULL, silent data loss); GUID -> canonical
...
| T10 | P1 | Async non-blocking DNS + IPv6 (blocking IPv4 getaddrinfo stalls the loop) | -- | net/eventedio | [x] |
...
T12 [x] 2026-08-03: observability landed. metrics.ky = string-keyed Registry (counters + gauges) with
numStr, snapshotProm (Prometheus text) + snapshotJson (deterministic via an insertion-order name list);
log.ky gains a tracing Span (startSpan/finish -> elapsed_ns log line + <name>_ns_total counter +
...
dbTimestamp/asTimestamp, and NULL-aware asIntOr/asLongOr/asDoubleOr/asTextOr/asBoolOr (db.ky); ORM
write side is update<T>(conn,table,obj,keyCol) + deleteBy(conn,table,keyCol,keyVal) (orm.ky); drivers
...
last/findIndex/any/all/indexOf/contains, case 235); config.ky (env-override layered config, case 236);
...
Kyte will consume EXTERNAL services (some over TLS 1.2, which the CLIENT role already supports -- M13,
...
Rationale: Kyte already serves TLS 1.3 (both roles, full spec) and connects over TLS 1.2 (client role).
...
the ones that exist are legacy middleboxes / ancient Java / Win7-era stacks, not Kyte's target. The
