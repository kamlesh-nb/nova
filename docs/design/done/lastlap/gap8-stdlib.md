# Gap 8: Nova Standard Library depth -- honest assessment

Assessed from the actual `.nova` source in `/Users/kamlesh/nova-lang/lang/src/lib/std`
(144 `.nova` files, 28,592 lines) on 2026-08-15. Where a claim is a plan or a guess it is
...
| Area | Files | Notes |
|---|---|---|
| `crypto/` | 40 | hash (md5/sha1/sha256/sha512/sha), mac (hmac/poly1305/ghash), aead (aesgcm/chachapoly), cipher (aes/aesctr/chacha20), ecc (p256/p384/x25519), kdf (hkdf/pbkdf2), rsa, x509, ocsp, crl, scram, base64, random, full TLS 1.2 + 1.3 client/server. Pure Nova (wolfSSL deleted per memory). |
...
Note: there is **no `decimal.nova` module**. `decimal` is a **compiler-native type** (decimal128
BID per memory `nova-decimal128`), surfaced in `serde/json.nova` as `decimalValue`/`asDecimal`.
...
- Stale/explanatory comments: `net/ev/epoll.nova` ("earlier note was stale"), `net/ev/iocp.nova`
  (Windows runtime not wired -- a real platform gap, but off the macOS/Linux primary target),
  `crypto/tls/13/handshake.nova` (only SHA-256 transcripts "for now"; SHA-384 suites are a follow-on
  -- a genuine TLS depth gap), `compress/deflate.nova` ("leak for now, a --asan pass will free them").
- Windows stubs returning -1: `os/windows/socket.nova`, `os/windows/sys.nova`, `process_windows.nova`
  (documented Windows-target incompleteness, consistent with CLAUDE.md "Windows is cross-compile only").
- Legitimate runtime panics for programmer error, not stubs: `collections/list.nova:14`
  (`List.at` out of bounds), `collections/map.nova:164` (`Map.at` key not found),
  `web/di.nova:59,109` (required service not registered).
- `concurrency/async_util.nova` "DEAD placeholders" -- **intentional**: `whenAny`/`selectAny` are
...
**collections/list.nova (82 lines, HAS @test, exercised by ~70 conformance cases).**
...
**collections/map.nova (305 lines, HAS @test, 3 conformance cases).**
...
**collections/set.nova (109 lines, HAS @test, 2 cases).**
Present: add/has/remove/size/forEach/toList/unionWith/intersection/difference. Missing `isSubset`,
`addAll`. Verdict: **adequate.**

**collections/deque.nova, heap.nova (HAS conformance case each, NO inline @test).**
...
**string.nova (700 lines, HAS @test, used by 63 conformance cases -- the most-tested module).**
...
Verdict: **broad and well-tested core; missing several everyday conveniences.** `str.nova` (40 lines) is
the separate borrowed-`Str` view type (ptr+len, zero-copy), not a duplicate -- it is the ORM/DB fast path.

**math.nova (418 lines, HAS @test, 4 conformance cases incl. case 244 trig/log depth).**
...
**datetime.nova (492 lines, HAS @test, 1 conformance case -- case 244 timezone parse/format).**
...
**text/regex.nova (533 lines, HAS inline @test at line 460 -- NOT reached by conformance import grep,
...
**serde/json.nova (433 lines, NO inline @test, but 17 conformance cases import serde.json).**
...
**net/url.nova (112 lines, HAS @test incl. IPv6).** `parse()` only -- returns
...
**parse-only and shallow.** (`web/url.nova` is a separate web-layer variant, HAS @test.)

**web/client.nova (435 lines, NO inline @test, 2 conformance cases).** HttpClient get/post/send, a
...
- **Effort (guess):**
  - Steps 1+3 (list/map/string/math/url convenience methods, each with a test): **~3-5 days.** Mostly
    straightforward Nova, watch for ARC ownership on new string-returning methods.
...
2. **Every stdlib file that exports a public API contains at least one inline `@test`.** Concretely:
   `for f in $(find src/lib/std -name '*.nova'); do grep -q 'pub fn\|pub struct' "$f" && ! grep -q '@test' "$f" && echo "$f"; done` returns **only** platform-internal files (`os/*`, `net/ev/*`, `mem/*`)
...
3. **Each crypto primitive has a KAT `@test`** asserting output against its published RFC/NIST vector,
   and the full `nova test` + conformance suite is green.
