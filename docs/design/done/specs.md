# Kyte Language — Specification & Reference
...
> and the native-default `kyte build`. Consult it for nothing.
...
Kyte is a statically-typed, compiled, ES6/TypeScript-flavoured language for **server-side services,
hypermedia apps, and WASM**. Compiler in Zig → LLVM IR; runtime in C++20 (Boost.Asio); **ARC, no GC**.

**Targets — ⚠️ the CLI defaults to `--wasm`:**

```
kyte app.ky --native -o app        # native — what you almost always want today
kyte app.ky --wasm   -o app.wasm   # wasm  — ⚠️ on hold (§12)
```

Compiling without `--native` and getting `Function 'kyte_test_fail' not found` means you built for wasm.

| Fact | |
|---|---|
| `zig build` (in `lang/`) | builds the compiler **and installs the stdlib into `~/.kyte/std`** |
| Import resolution | `src/std/…` relative to CWD, **falling back to `~/.kyte/std`** |
| ⚠️ **Consequence** | after editing `lang/src/std/*.ky` you **must** `zig build`, or anything compiled outside `lang/` silently uses the **old** stdlib |
| `KYTE_THREADS=n` | runtime worker-pool size (default: hardware concurrency) |
| `KYTE_KEEP_OBJ=1` | keep the intermediate `.o` (e.g. to relink under a sanitizer) |
...
- **`throw` / `try` / `catch`** — **REMOVED** (§5.5). Kyte has no exceptions; they never worked.
...
> encoding MongoDB's BSON decimal128 uses, so Kyte's `decimal` is wire-identical to BSON with no
...
>   slot is a pointer. (16 bytes cannot fit Kyte's 8-byte slots.)
...
>   (`kyte_decimal_add/sub/mul/div/mod/cmp`). Div-by-zero currently yields `0` (a Stage-2 stub).
...
documented idiom** for building a string from raw bytes (`string.ky:allocString` does exactly this).
...
> `38_optional_deref_guard.ky` (present-value paths) and the whole corpus being ASAN-clean.
>
...
> **Status:** LANDED 2026-07-17 — gated by `conformance/cases/33_error_union.ky` (ASAN-clean).
...
`map.get(k)` is absence. `parseInt(s)`, a DB round-trip, a socket read are failures. **Kyte has no
`null`** — it never has, and it never will; the `null`/`undefined` split is the worst thing JS/TS
inherited and Kyte is free of it. Keep it that way.
...
`catch` pairs with `??`: absence vs failure, same shape. Rejected: `!int` / `?int` sigils — Kyte
...
- **Alternative:** `f()!` / `f()?` to propagate (Rust/Swift-ish). Cheaper to explain to a .NET
  audience, but `?` collides visually with `?.`/`??`, which Kyte already uses for absence.
...
- **No stack traces.** `kyte_get_stacktrace()` returns an empty buffer and always has, so `throw`
...
- **No implicit conversion between error types.** Rust's `?` is ergonomic only because of `From`; that
  is trait machinery Kyte does not have. Convert explicitly, or use one error enum per layer.
...
- **Go `(T, error)`** — REJECTED, and measured (§6.6 and `route-handling-via-mediator.md` §8.D):
  ...
  Go's model works because Go's checker types multi-returns and Go has a GC; Kyte has neither.
...
rejection table. In Kyte *heap-allocated* and *reference-counted* are true of every `string`, `List`,
...
Kyte has exactly **two** binding keywords (ES6-style): `let` for a mutable variable, `const` for an
...
gated by `expect_fail/const_reassign.ky`. `let` is reassignable (`let i = 0; i = i + 1;`).
...
### 5.5 `try` / `catch` / `throw` — ❌ **REMOVED. Kyte has no exceptions.**

Reserved words (like `match`), so using one is a compile error that explains itself rather than
`undefined identifier 'throw'`. Gated by `expect_fail/throw_removed.ky` and
`expect_fail/try_catch_removed.ky`.
...
>
> 1. **The thrown value could not survive.** `kyte_throw` took a `long long`; codegen's catch bound
>    `zext(setjmp_res)` — an **i32**. The stdlib's only user, `web/di.ky`, did
>    `throw "DI Error: Service not registered: " + key`; `web/recovery.ky` caught it and logged
>    **`[RECOVERY] Caught exception: 8472`** — the string pointer truncated to 32 bits and
>    re-stringified as a number. `web/mediator.ky` had already stopped reading `err` and passed a
...
>
> Also gone: `kyte_throw`, `kyte_push_exception_frame`, `kyte_pop_exception_frame`, the
> `ExceptionFrame` stack, and the `_setjmp` declaration that existed only to serve them.
> **There are no stack traces either** — `kyte_get_stacktrace()` returns an empty buffer and always
...
writeFsFile(`dir/changed_${count}.txt`, "…");   // real stdlib usage (fs.ky:50)
```
...
```kyte
// map.ky:  pub fn forEach(self: Map<K,V>, fn: (K, V) -> void): void
...
### 6.7 `if_expr` / `block_expr` — 🔎 present; `string.ky` uses `if`-as-expression.
...
`run()` executes on a **`KYTE_THREADS`-sized pool** → genuine multi-core. ⚠️ `go` handles can be stored
...
> `concurrency/fiber.ky`'s `spawn` runs the closure **inline on the calling thread**
...
`15_name_resolution`. Record: `lang/repro/driver_alloc_churn_crash.ky`.
...
| 22 | ⚠️ **async scheduler: a rare residual race** | **Four scheduler bugs fixed 2026-07-15** (the `10_async_go` flake, ~20% → 0/50): a **lost wakeup** (re-scheduling while `running` was still set → the request was swallowed into `pending_resume` and never re-checked → `await` returned a stale pointer); `delete state` **under its own `lock_guard`** (unlocking a freed mutex); **`kyte_run()` returns on an *idle* io_context but the caller read the root's promise unconditionally** — idle ≠ done, now `kyte_run_root()` drives until genuinely complete and **aborts** on a lost wakeup rather than returning garbage; and a **stale waiter** — the early-done path skipped `take_waiter()`, leaking a registration in `g_waiters`, which is keyed by a **reused heap address**, so a later coroutine inherited it and the scheduler resumed a destroyed frame. Frames are now freed via `kyte_coro_release()`, serialised against the scheduler, not a bare `llvm.coro.destroy`. **Residual: 1 bad `await` in ~18,000 drives — real, not noise, and not one of the four.** Repro + method (ASAN is useless here; `KYTE_THREADS=1` is the control; lldb finds it): `lang/repro/async_scheduler_race.ky`. **Do not treat the scheduler as sound.** |
...
Detail: **`kyte-language-evolution-plan.md`** (L0–L6). Sequencing: **`kyte-readiness-roadmap.md`** §M2.5.
...
> — 1800 drives clean while the same binary crashed 3/20 natively); `KYTE_THREADS=1` is the control; a
