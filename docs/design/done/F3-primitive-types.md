...
> A declared type in Nova does not describe storage. There is exactly **one** runtime type — a machine
...
self-aligning (`align == the field's own size`), no struct-level alignment, no tail padding. Nova structs
...
`list.nova:12` declares `pub data: i32` and stores a 64-bit heap pointer in it. It does not truncate
...
| File:line | Field |
|---|---|
| `std/collections/list.nova:12` | `pub data: i32` |
| `std/collections/string_builder.nova:6` | `pub buf: i32` |
| `std/collections/array.nova:33` | `ptr: i32` |
| `std/concurrency/atomic.nova:7` | `pub ptr: i32` |
| `std/data/btree/client.nova:50` | `pub buf: i32` |
| `std/data/btree/protocol.nova:98` | `pub buffer: i32` |
| `std/fs.nova:12`, `std/io/file.nova:20`, `std/io/dir.nova:23` | `pub handle: i32` |
...
`left orelse right orelse "i32"` (:457), `bytes.*` → `"i32"` (:252), `nova_dir_*`/`nova_file_*` → `"i32"`
...
Driver: **BSON**. BSON's `decimal128` (type `0x13`) *is* IEEE decimal128, so this choice makes Nova's
...
3. **Nova has no operator overloading**, so `a + b` on `decimal` is **lowered by the compiler to a
...
**Nova targets web developers.** `__i32_to_string(n)` is compiler plumbing wearing a fixed-width type
...
- #2 `${i64}`/`${f64}` SIGSEGV — ✅ **fixed (stage 3, 2026-07-16)** via runtime `nova_f64_to_string`/`nova_i64_to_string`; the interpolation path no longer treats a number as a string pointer
...
1. ✅ **Overflow policy = trap in debug, wrap in release**, with explicit `+%`/`-%`/`*%` wrapping ops.
   *Implementation staging:* land **wrap semantics first** (arithmetic in i32 — the release behaviour, and
   what a 32-bit `int` observably IS); the debug-trap needs a debug/release build distinction Nova does not
...
- [x] `${f64}` / `${long}` case green (closes §10 #2) — `18_float_interp.nova`, 2026-07-16.
