# Nova Language Evolution Plan — L1–L6
...
>   "recommendation: take ICU, gate it behind `NOVA_HAVE_ICU`"). **Decide before starting either.**


Detail plan for the five language items raised on 2026-07-15, plus the i18n story a web-first language
needs. This is the **implementation detail**; `nova-readiness-roadmap.md` §M2.5 is the index that points
...
garbage length header — see `lang/repro/driver_alloc_churn_crash.nova`); (2) binary-safe strings
(`nova_from_bytes(ptr,len)`). L2/L4/L5/L6 all pile onto the string path — fix it before building on it.
...
1. L2 originally proposed *"add string interpolation"* + a `text.format`. **Nova has had ES6 template
   strings all along** (`lexer.zig:40`, `ast.zig:296`, used at `fs.nova:50`). The plan was inventing a
   shipped feature.
...
(Nova already leans that way), it is efficient on wasm, and it's honest. The alternative (`int` = 64)
...
> `text.format`. **Both were wrong — Nova already has ES6 template strings**, and `format` isn't needed.
...
Nova **already has** backtick template strings with `${}` interpolation — `lexer.zig:40-41`
(`template_string`, `interpolated_string`), `readTemplateString` on `` ` ``, `ast.zig:296`
(`template_expr`), parser at `1787/2075/2166`. It is used in the stdlib: `fs.nova:50` →
...
```cpp
char *nova_sha256(const char *input) { return nova_from_cstr(input ? "" : ""); }   // returns ""
char *nova_md5   (const char *input) { return nova_from_cstr(input ? "" : ""); }
```

…while `crypto.nova`'s own `@test` asserts `sha256("hello") == "2cf24dba…"` — passing vacuously.
wolfSSL is already vendored, built and linked (`deps/wolfssl/build/libwolfssl.a`, `-DNOVA_HAVE_WOLFSSL`);
wolfCrypt is sitting there unused.

**Blocker:** `nova_from_cstr` is NUL-terminated ⇒ it cannot carry raw bytes. Digests can dodge via hex,
but `randomBytes`/AES/sign cannot. Land `nova_from_bytes(ptr,len)` first.
...
let raw  = hash.sha256Bytes("hello");            // binary-safe (needs nova_from_bytes)
...
`options.h` against Asio). Every entry guarded by `NOVA_HAVE_WOLFSSL` with an **honest error** when
absent — never a silent empty-string stub again. Then `std/crypto/*.nova` + NIST KATs in the corpus.
...
An earlier draft said "implement a Thompson NFA in Nova (~weeks)". **That was wrong: Boost.Regex is
...
**Decision:** bind Boost.Regex for **native**, behind a thin Nova-facing API (`text.regex`) that hides
`boost::regex` entirely — so a future pure-Nova or ICU-backed engine can be swapped underneath without
...
Nova is a **web** language: the *first* thing a real app needs after routing is "render this in the
...
2. **Server-rendered HTML — which is Nova's whole point.** `CLAUDE.md` positions Nova for
...
`NOVA_HAVE_ICU`, mirroring the existing `NOVA_HAVE_WOLFSSL` pattern — full i18n + Unicode regex where
ICU is present, an honest degraded path (gettext plurals, no `u32regex`) where it isn't, and **never a
silent stub** (that is exactly how `crypto.sha256` came to return `""`). Keep the Nova-facing API
...
| Extraction tooling (`nova i18n extract`) | |

### Work

`text.locale` (tag parse/negotiate/fallback), `text.i18n` (catalog loader, ICU-style message parser,
CLDR plural rules, formatters), CLDR data as generated Nova tables (a subset, per-locale, additive), and
`nova i18n extract` to pull keys out of source/NSX. Depends on **L5** (UTF-8 + regex for the message
...
|---|---|---|
| **L-0** | binary-safe strings; **fix the string heap bug** | `driver_alloc_churn_crash.nova` passes; ASAN clean under churn |
