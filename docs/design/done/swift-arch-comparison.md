# Kyte vs Swift (swiftlang/swift) — architectural comparison + re-verified genuine gaps (2026-08-16)

Purpose: after scrapping the LLVM-emit optimiser, compare Kyte's pipeline against Swift's proven design
and re-derive Kyte's GENUINE gaps. Swift facts here are from architectural knowledge of the swiftlang/swift
...
**Kyte (current, post-scrap 2026-08-16):**
```
...
**Swift has a dedicated ownership-aware IR (SIL, in OSSA form) between Sema and LLVM. Kyte does not.**
...
In Kyte, there is NO such IR. Ownership is ANALYSED in sema (`ownership.zig` = a move/use-after-move balance
...
as opaque `kyte_retain`/`kyte_release` calls LLVM must not touch.

## 3. What that costs Kyte (both gaps share ONE root)
...
- **Soundness (was "gap 1").** Swift's OSSA VERIFIER proves at compile time that there is no use-after-move,
  no leak, no double-consume — every SIL function must pass it or the compile fails. Kyte has ownership
  ANALYSIS (ownership.zig) but not an ownership-SSA VERIFIER that gates the build; the actual safety net is
  the **ASAN corpus gate at RUNTIME**. So Kyte's memory safety is EMPIRICALLY TESTED (ASAN-clean on the
...
- **Perf (was "gap 3").** Swift's ARC optimizer lives in SIL because that is the only place ownership is
  explicit enough to safely remove retain/release. Kyte has nowhere to do it — which is exactly why the
  scrapped emit optimiser's arc_elision fired 0× and why LLVM (which can't see through kyte_retain/release)
  leaves all the refcount traffic in.

**So gap 1 and gap 3 are not two gaps. They are one: Kyte has no ownership IR.** Swift proves that a single
...
## 4. Where Kyte is genuinely OK vs Swift (don't over-scope)

- **Generics:** Kyte MONOMORPHISES (specialises every instantiation). Swift can specialise in SIL OR keep
  unspecialised generics behind witness tables. Kyte's mono is simpler and adequate — NOT a gap. (It does
  cost code size / compile time, but correctness + perf are fine.)
- **Value semantics:** Kyte already lowers value structs inline (Swift-style), done earlier. Fine.
- **Exclusivity enforcement (Swift's `@inout` law-of-exclusivity):** Kyte has no `inout`-style exclusivity
  model, so it does not need Swift's exclusivity passes. Not a gap for Kyte's semantics.
- **The AST→LLVM backend itself:** solid. LLVM O3 gives Kyte competitive scalar/loop codegen for free.
...
~100 optimisation passes, built by a large team over many years. A full Kyte SIL is a multi-YEAR effort and
...
1. **Promote the analysis Kyte already has into a minimal ownership IR.** ownership.zig (move-check) +
...
genuine-value item on Kyte's whole roadmap.
