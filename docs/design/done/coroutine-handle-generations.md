...
- `BTreeDriver.connect` returns `Connection` (the trait), so `let conn = await driver.connect(...)` is a
  ...
  `kyte_retain` of the underlying struct).
...
the new `expect_fail/trait_to_concrete_narrowing.ky` regression guard); orchestrator suite 13/13; the 15
...
additional, deeper await-lowering bug that produces a stale `kyte_sched_schedule` of an already-reaped
...
the paired memory note `kyte-conn-byvalue-async-arg-crash` for the live-debugging trail.
...
frame is freed (`kyte_coro_release` / the detached-reap in `reactor_finish`) and the allocator recycles the
...
- reactor timers (`kyte_reactor_set_timer(handle, ms)`),
...
`/tmp/connpass.ky` shape (needs a live NovaDB on 127.0.0.1:3009):
...
Crash: `SIGBUS` at `kyte_reactor_resume + N` (which inlines `reactor_pump`), fault address in the code
segment (a garbage resume-fn pointer). Connect+query in the SAME coroutine (`/tmp/connIn.ky`) is fine; the
...
pattern, so this blocks connection pooling on the Kyte reactor.

Note this is a SEPARATE bug from the NovaDB server WAL data race fixed in `btree@6600c4d` (that was a
missing lock on the server side; this is a client-side Kyte-runtime scheduler issue).
...
clearing it each `kyte_reactor_batch_begin`), always-track reaps, add a check to `reactor_pump` (only
`kyte_reactor_resume` had one), and erase-on-rebirth (`mark_reborn`) wherever a handle is scheduled.
...
extern "C" unsigned long long kyte_coro_gen(long long h) { return coro_gen(h); }
...
- `kyte_coro_release(handle)` (the awaiter reaps a child) -- already calls the old `mark_reaped`; replace
  with `bump_gen(handle)` before the destroy.
- `reactor_finish` detached-reap branch -- same.
- `kyte_sched_schedule_detached` already-done branch -- currently reaps WITHOUT any mark; add `bump_gen`.
  (This missing mark is a pre-existing gap.)
- Audit `kyte_run_root` / the block-drive root reaps: a root frame is driven directly, not by a stored
...
1. Reactor OPS. Widen the op record: add `gen i64` at offset [40], `OP_SIZE` 40 -> 48 (all four
   ...
   token)` captures `gen = kyte_coro_gen(token)` and writes it at [40]; add `opGen(op)`. At completion in
   each backend's `pollComplete` (where it reads `opToken` and marks the op done): if
   `kyte_coro_gen(opToken(op)) != opGen(op)`, the submitting coroutine was reaped since submit -> DROP the
   completion (do not deliver the token to the resume path; free/abandon the op per that backend's rules).
   This is the primary fix: it stops a stale socket completion from ever reaching `kyte_reactor_resume`.

2. Run queue `g_rq`. Change the element type from `long long` to `struct { long long h; uint64_t gen; }`.
   `kyte_sched_schedule(h)` and the waiter-push in `reactor_finish` capture `coro_gen(h)`. `reactor_pump`
...
4. Timers. `kyte_reactor_set_timer(handle, ms)` -- the timer's completion also resumes `handle`. Capture
...
5. Reactor `udata` / direct `kyte_reactor_resume(token)`. `kyte_reactor_resume` already receives a bare
   token from the pump loop. With item 1 in place, a stale SOCKET token never reaches it. For defense in
   depth, `kyte_reactor_resume` can also take an optional expected-gen and skip on mismatch, but items 1-4
...
- `src/runtime/concurrency.cpp` -- generation table + `kyte_coro_gen`; `bump_gen` at reap sites; `g_rq` and
  `g_waiters` element types; gen checks in `reactor_pump` (and optionally `kyte_reactor_resume`); remove the
  now-subsumed per-batch `g_batch_reaped` machinery (or keep as a fast-path, but the generation table is
  authoritative).
- `src/std/net/eventloop_darwin.ky`, `eventloop_linux.ky`, `eventloop_windows.ky` -- `OP_SIZE`
...
- `src/std/net/reactorio.ky` -- no signature change (makeOp gains the capture internally); confirm the
  timer arm/cancel path carries a gen.
- `src/runtime/kyte_abi.h` -- declare `kyte_coro_gen`.
...
3. `conformance/run.sh --asan` (329) -- ASAN is the authority for the use-after-free class; it must be
   clean. Note ASAN cannot link reactor programs today (the asan runtime lacks `kyte_op_alloc`); wire the
   reactor objects into the asan build, or validate the reactor path with `KYTE_CRASH_TRACE` + the stress
   repros instead, and state which.
4. The direct repros: `/tmp/connpass.ky`, `/tmp/connU.ky` must PASS; `/tmp/locq.ky`,
   `/tmp/connIn.ky` must stay passing.
...
Diagnostics that cracked this: `KYTE_TRACE=<path>` (per-coroutine schedule/finish/resume log) and
`KYTE_CRASH_TRACE=1` (backtrace + fault address on an alternate signal stack). macOS has no `timeout`; use
...
(`KYTE_TRACE`) shows why, and it changes the picture:
...
- The crash is in `reactor_pump`: a handle X finishes (`finish h=X`), and LATER `sched->rq h=X` +
  ...
  it (`skip-stale-gen` count 0), because the stale `kyte_sched_schedule(X)` happens AFTER X was reaped, so
...
So the real defect is UPSTREAM of the queue: something calls `kyte_sched_schedule` with a handle whose frame
...
2. Focus on `buildTraitVtableCall` (async trait-method await) + `awaitedCallHandle`: does the child handle it
   ...
   `kyte_sched_schedule`, which then no-ops on `raw_coro_done`, leaving the waiter chain in an odd state).
3. Consider whether the fix belongs in the codegen (capture the child's gen at await time when it is known
   live, and pass it to `kyte_sched_schedule` so a post-reap re-schedule carries the OLD gen and is gated) --
...
`kyte_register_waiter(child)` + `kyte_reactor_detach`; dead at reap) plus a log of the resume-fn pointer
...
`kyte_sched_schedule`" reading (that address WAS a live recycled coroutine, resumed with matching gen; the
...
a `delete()` destructor, see memory `kyte-ptr-field-arc-ownership`) and its `AsyncStream` (socket fd) into
...
`KYTE_ASAN=1 kyte test /tmp/connpass.ky` (the asan runtime just needed a fresh `KYTE_ASAN=1 zig build`;
`runtime.cpp` already includes `core.cpp`, so `kyte_op_alloc`/`kyte_op_free` are present -- the earlier link
...
- The runtime's built-in double-release detector (`KYTE_ARC_AUDIT=1 KYTE_ARC_DUMP=1`,
...
- Instrumenting `kyte_release` to log every destructor that runs (dladdr on the dtor fn ptr) shows the freed
...
Dumped the pre-CoroSplit IR (`__kyte_test.ll`, written by the test path) and ran the LLVM coro passes
...
reference to `conn` for the duration. Harness = `KYTE_ASAN=1 kyte test /tmp/connpass.ky`; opt-split recipe
...
2. Inspect how a struct with a raw `ptr` field + `delete()` destructor is copied when passed BY VALUE into an
   ...
   destructor-bearing structs (relates to memory `kyte-ptr-field-arc-ownership`, `kyte-semantics-from
