# Kyte package manager — locked design (2026-08-16)
...
> fetch trusts each package's declared dep list) remains the documented out-of-scope limitation; `kyte
> vendor` is the recorded first future step (remaining-gaps-design.md Gap 8).


The deliberately-small package manager for Kyte. It is **Cargo-flavoured** in identity (clean by-NAME
...
Why this is feasible cheaply: Kyte's symbol mangling is **path-derived** (`getModulePrefix` builds the
...
```jsonc
{
  "name": "my-lib",
  "version": "1.2.3",
  "type": "library",                               // gates `kyte publish`
...
- `repository` is optional; a leaf app omits it. `kyte publish` requires it.
...
1. **`resolved` SHA from the lockfile, if present** → check that out. Immune to a moved tag or a new
   default-branch push. **`kyte build` / `kyte test` always honor the lock and never rewrite it.**
...
Only the explicit commands `kyte get` and `kyte update` are allowed to MOVE a locked SHA.

## 4. Fetch + cache
- Cache dir: `~/.kyte/cache/<name>-<sha8>`, where `<name>` is the dependency's DECLARED name (from its
...
- Fetch: a pinned target (locked SHA or `#ref`) requires history, so `git clone <url> <tmp>` then
  ...
  fetched `<tmp>/project.json` `name` and the resolved SHA, then move `<tmp>` to `~/.kyte/cache/<name>-<sha8>`
...
- When resolving `import X` inside a source file, find the file's **owning package** = the nearest
  ...
  its locked `resolved` SHA → the cache dir `~/.kyte/cache/X-<sha8>`.
- A dependency's files live under `~/.kyte/cache/A-<shaA>/…`; when A itself does `import X`, the owning
...
| Command | Contract |
|---|---|
| `kyte get <url[#ref]>` | add the dep to `project.json`, resolve ref→SHA, fetch (+ transitive), write the lock. |
| `kyte restore` | fetch every dep (direct + transitive) at its **locked SHA**; no re-resolution. Fails loudly on a missing lock entry only if the ref is also absent. |
| `kyte update [dep]` | the ONLY command that moves a locked SHA to the ref's current tip. With no arg, updates all pinned deps; with a dep, just that one. Rewrites the lock. |
| `kyte publish` | tag the current version and push it (see §8). |

Auto-fetch: `kyte build` / `kyte test` call the same resolver (honoring the lock, §3) before compiling, so
`git clone <app> && kyte build` just works.
...
7. Print the consume line: `kyte get <repository>#v<version>`.
...
1. Repos A→B→C with tagged releases; `kyte get A#v1` fetches all three; lock records three entries with
   resolved SHAs AND names taken from each repo's own `project.json` (not the repo name).
2. Move a tag; `kyte build` still checks out the LOCKED SHA (reproducible); the lock is unchanged.
3. `kyte update` moves it; the lock changes.
...
6. `kyte publish` on a library creates `v<version>` and pushes to origin; refuses on a re-publish.
