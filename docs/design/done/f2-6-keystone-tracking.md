...
| W9 | Cut `typeRefToString` sites (drops `.optional` and scope) | 🟡 | 60 call sites. Known lossy: dropped `.optional` on the mono NAME (`renderLegacy .optional` rendered `int \| undefined` as `int`), which **collided `List<int \| undefined>` with `List<int>`** at the monomorphised symbol → boxed-vs-raw layout mix → UAF; **FIXED** by rendering value-optionals distinctly + mangling `\|` (A3-read, fuzzer-found, cases 286). Also drops module scope (enums/structs now via `scopedTypeName`/`renderLegacy`) | audit each of 60; route through TypeId where identity matters |
...
| W11 | Codegen soundness fuzzer (gen well-typed → compile → run → oracle) | 🟡 | **Landed + broadened** `conformance/codegen_fuzz.py` + `.sh`. Surface: **int + long** arithmetic (`+ - * / % & \| << >>`, 32/64-bit wrap, trunc div/mod, narrowing casts), **six comparisons**, **structs** (construct with known field values, read back and combine → field-offset codegen), and **generics** (`Box<T>`, free `id<T>` explicit + inference, `outer<T>→inner<T>` composition; T ∈ int/long). Two program shapes (value `equalInt`, boolean `isTrue/isFalse`). **Teeth proven** on arith, compare, and struct shapes. All match on the fixed compiler across seeds. Found a real loud gap while broadening: `let x: T = generic_inference_call(v)` (inference into a typed let) is rejected — recorded in the readiness report, avoided by the generator. Broadened again to **value-optionals in containers** (`List<T\|undefined>` push a mix of values/undefined across a growth boundary, read each `at(k) ?? default`), which **found and drove the fix of a real soundness bug (A3-read)**: `List<int\|undefined>` monomorphised to the same symbol as a plain `List<int>` because `renderLegacy .optional` dropped the `.optional`, mixing boxed and raw layouts → UAF on a ≥2-element read. FIXED (render value-optionals distinctly + mangle `\|`); the multi-element valopt template is now green and guards it (case 286). | next: ownership/ASAN runs, nested structs, maps/sets of value-optionals, CI at scale |
