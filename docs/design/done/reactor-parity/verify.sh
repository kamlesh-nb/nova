#!/usr/bin/env bash
# Runtime-verify the deadline-timer path (kyte_reactor_set_timer + opDone + abandonOp) that the
...
echo "== macOS / kqueue =="; kyte "$HERE/deadline_parity.ky" -o /tmp/dlp_mac && /tmp/dlp_mac
kyte "$HERE/deadline_parity.ky" --target linux-x86_64 -o /tmp/dlp_linux
echo "== Linux / epoll ==";     docker run --rm -v /tmp/dlp_linux:/dlp:ro debian:stable-slim /dlp
echo "== Linux / io_uring =="; docker run --rm -e KYTE_REACTOR=uring -v /tmp/dlp_linux:/dlp:ro debian:stable-slim /dlp
