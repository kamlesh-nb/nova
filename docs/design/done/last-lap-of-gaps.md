...
| 3 | Optimiser (SCRAPPED 2026-08-16 → Swift-SIL) | Low for beta, High for the vision | **SCRAPPED.** The HIR/MIR/LIR LLVM-emit optimiser delivered ~0 optimisation (LLVM O3 does the generic passes on both paths; the one non-redundant pass arc_elision fired 0/44). Removed to discards/optimiser-2026-08-16 (57da4cf). New direction = Swift-SIL ARC optimisation, go/scrap-gated on a MEASURED perf delta before any coverage. See docs/design/sil-arc-optimiser-direction.md. Prior (now-historical) detail below: | replaced by SIL plan | [sil](sil-arc-optimiser-direction.md) |
| 3-old | (historical) Optimiser completion (perf) | Low for beta, High for the vision | ~10-12% real emit coverage, perf ~0% realised. Measured 2026-08-16: arc_elision 0/44 (threading tight; perf needs unbuilt borrow-skip), 895 shadow removals + 7 inlines but only ~11% emitted, callee-res 45.5%. **B6 value-optional emit STARTED (multi-session):** foundation (bcfe63e) + return-boxing DONE+verified AST==EMIT/ASAN/ARC (f14f793, case 371). Remaining B6: params/locals/narrowing; then B7/closures/async/default-on/borrow-skip. Honest: ~15-25 verified slices total, one value-repr slice per focused session | months (multi-session, per-slice verifiable) | [gap3](lastlap/gap3-optimiser.md) |
