...
Reference workload: the Postgres pizza-catalog web app (`plancksystems/perf/compare/kyte`), one typed
...
Earlier per-core ratios were unfair on two counts: different payload sizes (Rust/Go shipped 228 KB, Kyte
194 KB) and Kyte-single-core vs peer-multi-core (which is *oversubscribed* on this 4-core box + co-resident
oha, depressing the peer's per-core). Both are now fixed: the Go and Rust apps were edited to emit the
**exact same 194,739 bytes** as Kyte (same escaping set, same card markup — a local benchmark change, not a
product change), and each is measured **single-core** (`GOMAXPROCS=1`, `TOKIO_WORKER_THREADS=1`, Kyte's one
reactor). oha c=50, `ps -o %cpu`.

| | rps | CPU | **rps/core** | vs Kyte |
|---|---|---|---|---|
| **Rust** (1 tokio worker) | ~4,300 | ~87% | **~4,300** | 1.8× faster |
| **Kyte** (1 reactor) | ~2,360 | ~88% | **~2,360** | — |
| **Go** (`GOMAXPROCS=1`) | ~1,670 | ~95% | **~1,670** | 1.4× slower |

Honest ranking per core: **Rust > Kyte > Go.** Kyte is **~1.4× faster than Go** and **~1.8× slower than
Rust** (≈55% of Rust). The earlier "Kyte ~84% of Rust" was a measurement artifact (bigger payload +
...
oversubscription tax that Kyte's single-reactor model sidesteps. Kyte's per-core number is the clean one.
...
per request is:
1. **`string` FIELDS.** Kyte has ONE string type: owned, heap, ARC-headered. Every `getText` / interpolation
   / concat mints an owned heap string. ~600 accessed cell strings + 200 `Row.raw` + render temporaries =
   the bulk of the churn (`kyte_bytes_alloc` + `kyte_release`).
...
constant template chunks (string literals baked into the binary — Kyte already has these as immortal
literals, the L6 negative-refcount ones). Kyte's single owned string forces ~600 heap allocs where Rust does
...
whole-language borrow checker — it destroys Kyte's ES6/TS ergonomics, which is the point of the language.
See P10/P11 below.
...
- **gzip** — ruled out: the pure-Kyte DEFLATE burns ~60% of a core and degrades throughput.
...
| Ai | Inline ARC release fast-path (skip call for rc<0) | 2 | compiler (arc.zig) | ❌ reverted | +? | **profile ARC 20%→8%, wall-clock NEUTRAL** | `kyte_release` 521→171 in profile, but workload is PAYLOAD-bound now so no rps gain, AND it taxes every positive-rc release (DB decode has thousands). Corpus+ASAN green but reverted — revisit for small-response/JSON workloads |
...
| P9 | **SWAR escape scan** for `escapeHtmlInto` | 2 | stdlib+runtime | ✅ | part of the ~40% byte-work | **escaping 277→~109 samples (−60%)** | `kyte_html_find_meta` (runtime SWAR, 8 bytes/iter branch-free, exact has-zero-byte trick) returns next-metachar index; `escapeHtmlInto` drives off it — 1 bulk scan per clean value, not per byte. Byte-identical, corpus 319/320. Strictly faster on ALL inputs, NO tax on any path (unlike the reverted ARC inline) → KEEP. General stdlib win (every HTML render). rps ~neutral here (payload-bound) |
| **P10** | **Borrowed string type (`str.Str` = ptr+len)** | 3 | language + compiler + stdlib | ✅ end-to-end | **+~15% cumulative** | **~2,360 → ~2,725/core** | Own module `str.ky` (`Str` value struct, `str.of/sub/raw`, `Str.toOwned`); `StringBuilder.appendBytes`; `escapeHtmlIntoView` (escape from a borrow, ZERO owned string); `DbValue.asView`/`Row.getView` (borrow the P4 cell). **NSX codegen**: `{v}` / `attr={v}` where `v` is a `Str` auto-escapes via the value-struct-arg lowering in `jsxAppendExpr` (fix: match `t=="Str"` and pass `val` DIRECTLY, NOT i64-coerced — that was the garbage). **App**: `ProductView` holds `Str` borrowing `rs.Row.raw`; render mints ~0 owned cell strings. Byte-identical, 100% success. Kyte now ~63% of Rust/core (was ~55%). NOT a borrow checker |
| **P11** | **Compiler escape analysis → stack-alloc / ARC-elide + safe arena** | 3 | compiler | ✅ analysis + Stage C/D codegen (default-off) | closes the allocation half; makes P7 safe | REPORT + `KYTE_ESCAPE_REGION` | **ANALYSIS DONE, SOUND + PRECISE** (`src/sema/escape.zig`, behind `KYTE_ESCAPE_SHADOW`): per-function escape walk + interprocedural `methodSelfEscapes` FIXPOINT + assignment/field-store handling + **owned-field/element PRECISION** (typeOf2 + isOwned: a primitive `p.x` read does NOT escape p; an owned `p.name` does) + async-conservative. Sound UNDER-approximation (never marks a truly-escaping object local); non-empty. **Stage C/D codegen LANDED, default-off behind `KYTE_ESCAPE_REGION`:** `escape.isRegionSafe(fn)` = sync + >=1 heap alloc + ZERO escaping → codegen wraps the function body in a scope region (`kyte_region_current/new/set` in the prologue at `declarations.zig`; `kyte_region_set(saved)+kyte_region_free(r)` in `releaseLocalVariables` = the universal pre-return hook, so every exit is covered; entry-block SSA dominates all returns). Region objects carry the immortal sentinel refcount so per-local releases are no-ops; the single free reclaims them all in O(1). **GATED:** corpus 320/321 (flag off, only inapplicable-off-Linux `189`), ASAN corpus + `str_borrow` GREEN with `KYTE_ESCAPE_REGION=1` (4 region-safe fns exercised, no UAF). Kept default-off pending a REAL-workload reached-after-scope gauge before flipping on; C's stack-alloc form of the same set + D's fold into the P7 blanket async arena remain the follow-ons. |
...
Cumulative note: byte-matched single-core, Kyte ~2,360/core = ~55% of Rust (~4,300), ~1.4× Go (~1,670).
...
Detail in memory `kyte-per-core-perf-vs-rust` and `kyte-per-request-arena`. All corpus-green (319/320);
...
Cells hold `(buf, off, len)` into a **retained** response buffer instead of an owned copy; a Kyte string
...
rolling 64KB reader). Kyte strings need an 8-byte header, so a borrowed cell must materialise-on-read
...
`kyte_bytes_persist`, `mem/region.runStr` synchronous scope) + **generation-validated per-coroutine
...
- **Inline Kyte SWAR** — do the wordwise test in Kyte over `long` reads (no call). Keeps it in the language.
- **A SIMD runtime intrinsic** — `kyte_html_find_meta(ptr, len) -> first-meta-offset` vectorised in C++.
  CAVEAT (measured): a per-interpolation C FFI *call* (`kyte_html_scan`) was tried and was SLOWER than the
...
Corrected diagnosis (2026-08-11, byte-matched): Kyte's per-request churn is NOT struct boxing — `ProductView`
/`DbValue`/`Row` are value structs stored inline in RawBuffers (M-1/2/3 + M-10 did their job). The churn is
(1) **owned `string` fields** — Kyte has ONE string type, owned+heap+ARC, so every materialised cell / concat
...
## P10 — Borrowed string type (`str.Str` = Kyte's `&str`)
...
- `src/std/str.ky` (own module, registered in `std_modules`): `pub struct Str { ptr, len }` (a value
...
- `web.response.escapeHtmlIntoView(sb, v: str.Str)` — HTML-escape straight from a borrow (reuses the P9 SWAR
  `kyte_html_find_meta`), minting NO owned string.
- `DbValue.asView()` / `Row.getView(i)` — borrow the P4 lazy cell as a `str.Str` (points into `Row.raw`).
- Conformance `str_borrow.ky` (whole / slice / clean-passthrough / toOwned) — 4 asserts, corpus + ASAN.
...
"non-ARC string" idea. NOT a whole-language borrow checker (which would trade away Kyte's ES6/TS ergonomics).
...
away Kyte's ES6/TS ergonomics, which is the reason the language exists.
...
- **Stage A — the analysis pass (no codegen change).** A per-function dataflow that computes, for each heap
  ...
  changes nothing.
- **Stage B — shadow validation (gauge, like `KYTE_SEMA_SHADOW`).** Run the analysis on the whole corpus and
