# Nova Async Runtime Plan — Asio + C++20 stackless coroutines (multi-core)
...
fastest and cleanest for an **I/O-centric** runtime, but they require the **Nova
compiler to emit coroutine lowering** (Nova compiles to LLVM IR, so Clang can't do
...
- The scheduler is a single `asio::io_context`, **`run()` on N threads** (a thread
  pool sized to cores). Coroutines are distributed across all cores; different Nova
...
`lang/src/runtime/`:
- `nova_abi.h` — frozen ABI (8-byte header, i64-pointer discipline, all ~104
  `extern "C"` symbols).
...
- `concurrency.cpp` — **synchronous shim** (spawn runs inline; std channels; entry
  calls `__nova_main`). Stable, corpus-green. This is the shipping default until the
  coroutine path is proven.
- Build: **prebuilt** into `~/.nova/lib/libnova_runtime.a` by `build.zig` (compile
  once; `nova build` just links it — was 7s/build, now sub-second). `BOOST_PREFIX`
  configurable per platform. `main.zig` links `-lnova_runtime -lboost_fiber
...
- ✅ **M0 — feasibility spike: PASSED (gate cleared).** Wired
  ...
  Verified: (1) the pipeline runs cleanly on real Nova code — **corpus 18/18 still
  ...
  `observable=2` after opt-lower + clang-link + run. **Residual CLOSED:** Nova links
  `/opt/homebrew/opt/llvm/lib/libLLVM.dylib` **v21.1.7 — the same LLVM `opt` used**,
  so the toolchain lowers coroutines identically. Only workstream C (Nova *codegen*
...
- **A — arena / string cleanup. HALF DONE (alignment landed; arena-drop scoped).**
  - **Alignment: DONE + validated (corpus 18/18; string stress 11/11; json/yaml/
    ...
    from the inventory + 2 inline ones in data/btree/driver.nova) from the
    ...
    `nova_bytes_alloc(len)` already writes the 8-byte ARC header (refcount @base+0,
  ...
  - **Arena-drop: DONE (arena off by default; corpus 18/18 + all string/serde/
    ...
    `-DNOVA_DROP_ARENA`, so `nova_bytes_alloc` always mallocs a real refcounted
  ...
  - **Container/ARC reconcile (follow-up, DONE this round):**
    - **Map "bug" was misuse, not a codegen bug.** `Map<K,V>(initialCap, hashFn)`
      ...
      `conformance/expect_fail/constructor_arg_count.nova`. Correct usage
      (`Map<string,i32>(16, string.hash)`) verified 6/6. (Int-keyed maps/sets are
      usable via `i32Hash` in set.nova.)
...
- **C — codegen coroutine lowering. SPIKE DONE (end-to-end through Nova codegen,
  ...
  - Runtime: `nova_coro_alloc/free` (nova_abi.h + alloc.cpp, plain malloc/free —
    frames are NOT ARC objects).
  - Codegen decls: `setupCoroutineSupport` declares `llvm.coro.*` via the
    intrinsic-ID path (LLVM builds the token-typed signatures) + `nova_coro_*`.
  - `FunctionInfo.is_async` threaded; async ramp returns i64 (the coroutine
    handle), marked `presplitcoroutine`.
  - Prologue (`emitCoroPrologue`): promise alloca + coro.id/size/`nova_coro_alloc`/
  ...
  - Epilogue (`emitCoroEpilogue`): final suspend + trap + cleanup (`coro.free`→
    `nova_coro_free`) + `coro.end` + `ret handle`.
  ...
  - **Real `await` suspension: DONE (corpus 18/18 green).** Validated the
    ...
    - `await inner()` (expressions.zig `buildAwait`): emit inner's ramp (handle, at
      ...
      handle into inner's `promise.waiter`; `nova_sched_schedule(inner)`; emit a
      mid-body `coro.suspend` (switch → `await.resume` / suspend / cleanup); on
      resume read inner's `promise.result` and `coro.destroy` it.
    - Epilogue: on completion, if `promise.waiter != 0`, `nova_sched_schedule(waiter)`
      before the final suspend.
    - Minimal scheduler: `nova_sched_schedule`/`nova_sched_next` (single-threaded
...
- **D — Asio runtime. IN PROGRESS (scheduler + timer awaitable DONE).**
  - **Enabler proven:** C++ can resume Nova's LLVM coroutines directly via the
    switched-resume ABI — the frame's first two pointers are resume/destroy, a null
    resume pointer means done (verified against Nova-compiled coroutines). So Asio
  ...
  - **D-step-1 (Asio io_context scheduler): DONE.** Replaced the M3-C FIFO with
    `boost::asio::io_context` (concurrency.cpp). `nova_sched_schedule(h)` posts a
    resume handler; `nova_run()` = `io_context.run()` + `restart()`. Codegen's
    `buildDriveAsyncCall` now emits `schedule(root) + nova_run()` (no more codegen
  ...
  - **D-step-2 (timer awaitable): DONE.** `await sleep(ms)` lowers to a NON-blocking
    timer: `nova_await_timer(self_handle, ms)` arms an `asio::steady_timer` that
  ...
  - **D-step-3 (multi-core): DONE (infrastructure + race-free; parallelism payoff
    needs D-4 spawn to observe).** `nova_run()` now drives `io_context.run()` on a
    pool of N threads (`NOVA_THREADS` env override, else `hardware_concurrency`
    ...
    (`nova_register_waiter(child, parent)`, a mutex-guarded map), and the RUNTIME
    schedules the waiter *after* the child's resume fully returns
    (`nova_sched_schedule` post-resume: `if done → take_waiter → schedule`). Codegen:
    `await child()` calls `nova_register_waiter` before scheduling the child; the
    ...
    async/await/timer suites green on NOVA_THREADS=1/4/8; corpus 19/19. NOTE: with no
  ...
  - **D-step-4 (concurrent tasks — `go` + Future): DONE.** New keyword **`go
    ...
    `nova_sched_schedule`, return handle). **`await <future>`**: `buildAwait` now
    falls through (when the operand isn't `sleep` or a direct async call) to
    `buildAwaitFuture` → `nova_await_future(future, self)` which, **under the waiter-
    ...
    on NOVA_THREADS=1/4/8 ×3 runs. Corpus 20/20 (added
    `conformance/cases/10_async_go.nova`).
  ...
  - ✅ **RESOLVED 2026-07-17 — and there was never an "atomic/closure-capture defect".
    ...
    The real bug was `nova_atomic_cas_i32`, which discarded the result of
    ...
    That made `atomic.nova`'s own `test_atomic_i32` fail, and `nova_test_fail` calls
    ...
    **Why it survived:** (1) no conformance case imported `concurrency.atomic`, so every atomic
    ...
    as a failure in another. Both fixed: `nova_test_begin` now names the running test, and
    `conformance/cases/31_atomics.nova` gates all of it (shown to fail before the fix).
    Also fixed alongside: `nova_atomic_cas_i64` declared `int32_t desired` while codegen passes
  ...
  - **D-step-5 (sockets + wolfSSL TLS): DONE — real HTTPS with verify_peer works.**
    Implemented the `nova_socket_*` + `nova_tls_*` ABI (io.cpp), replacing the stubs:
    - **Sockets:** blocking POSIX (`getaddrinfo`/`socket`/`connect`/`bind`/`listen`/
      ...
      `nova_close_fd`). `send` uses the Nova string length (`s-4`), binary-safe.
    - **TLS via wolfSSL** (`#ifdef NOVA_HAVE_WOLFSSL`): `nova_tls_new` builds a client
    ...
    - **Build wiring:** build.zig builds the vendored wolfSSL static lib if missing
      (cmake) and compiles the runtime with `-DNOVA_HAVE_WOLFSSL -Ideps/wolfssl
  ...
  - **D-step-6 (channels + async socket I/O + cleanup): DONE — workstream D COMPLETE.**
    - **Async channels** (concurrency/asyncchan.nova free-fn API): `chanNew/chanSend/
      chanRecv/chanFree` over `nova_chan_*`. `recv` is awaitable — `await chanRecv(ch)`
      lowers (buildChanRecv) to a park+retry loop: `nova_chan_recv` under a mutex
      ...
      element types are a follow-on. Kept in a standalone module to dodge channel.nova's
      pre-existing transitive `fiber` import error.)
    - **Non-blocking async socket I/O** (net/asyncio.nova): `await socketRecvAsync(fd,
      ...
      (buildAsyncIo → offload + buildAwaitSuspend + nova_io_take_result). Verified:
    ...
    - Corpus 21/21 (added `conformance/cases/11_channels.nova`).
  - **STILL open (outside D):** ~~the separate `test_fiber_execution` atomic/closure bug~~
    ✅ **RESOLVED 2026-07-17 — it was `nova_atomic_cas_i32` returning the expected value
    ...
    **Also open and newly recorded:** `nova_chan_new` ignores its `capacity` argument
  ...
  - **TLS DECISION: wolfSSL, NOT asio::ssl/OpenSSL** (matches CLAUDE.md "wolfSSL
    ...
    ABI — `nova_tls_new/handshake/write/read/free` (io.cpp, currently stubs) — so
    ...
    force-rebuild: `clang++ -std=c++20 -O2 -pthread -DNOVA_DROP_ARENA -c
    -I/opt/homebrew/include src/runtime/runtime.cpp -o ~/.nova/lib/nova_runtime.o &&
    ar rcs ~/.nova/lib/libnova_runtime.a ~/.nova/lib/nova_runtime.o`.
...
### New ABI symbols (added to nova_abi.h when C lands)
`nova_coro_alloc(i64)->i64`, `nova_coro_free(i64)`, `nova_coro_resume(i64)`,
`nova_coro_schedule(i64)`, `nova_await_timer/channel/socket_*`, `nova_spawn_task`,
`nova_future_await`. The existing blocking `nova_channel_*`/`nova_concurrency_*`
...
- **Arena is load-bearing** (blocks multi-threaded async): `string.nova`'s
...
single-threaded (NOVA_THREADS=1 → 100/100 concurrent), but multi-threaded it
corrupted (NOVA_THREADS=4 → ~19/100; =8 → ~4/100). A raw Python concurrent client
...
`nova_sched_schedule` posts resumes onto `state->strand`, and `nova_arecv`/`nova_asend`/
`nova_aaccept` `bind_executor(state->strand, ...)` their completions. Each connection
= one coroutine = one socket, so a per-coroutine strand serializes that socket's ops
across threads. **Verified:** 100/100 and 500/500 concurrent at NOVA_THREADS=1/4/8
(~7500 req/s, Python-client-bound); corpus 21/21; async/channel/timer suites green at
NOVA_THREADS=8. (Superseded the notes below — the RST/close theories were wrong; a bare
...
`nova_sched_schedule_detached`, which self-destroys the coroutine frame on completion
...
The scalable async server (D-7: `net/asyncio.nova` `serverListen`/`aaccept`/`arecv`/
...
- Diagnosis so far: the *server* handles every connection; the *response* doesn't
  ...
  `shutdown(send)` before `close` in `nova_aclose` (concurrency.cpp) — **did NOT fix
...
- Next hypotheses to check: (a) `async_write` completes but data isn't actually
  ...
  build a native server binary (`nova server.nova --native -o server`), run it,
...
root also has no waiter but its promise is read after `nova_run` — must distinguish
"detached go task" from "block-driven root" (e.g. a detached flag, or the root uses a
distinct drive path). Runtime hook point: `nova_sched_schedule` post-resume, where
...
- `app.nova` template (`nova init app`) improvements (user flagged it as "different
  than what it is").
...
- ✅ **Async client connect (`aconnect`) DONE.** `await aconnect(host, port)` → socket
  ...
  (net/asyncio.nova + `nova_aconnect`). Verified: async client fetched example.com:80
  (`aconnect`→`asend`→`arecv`) 5/5. So the full async socket set — accept/connect/
  recv/send/close — is complete and strand-safe.
- ⏳ **TLS-over-async server** still TODO: `nova_tls_*` is blocking wolfSSL over a raw
...
- `asend` currently sends a Nova string (length at s-4). Add async connect
  (`aconnect(host,port)` for a scalable client) if the web *client* needs it.
- TLS-over-async: `nova_tls_*` is blocking wolfSSL over a raw fd; a TLS *server*
...
- Async channels carry **i32** only (`nova_chan_*` + asyncchan.nova). Generalize to
...
- ✅ **`test_fiber_execution` — RESOLVED 2026-07-17, and it was never a fiber/closure/atomic-capture
  bug.** It was **not failing; it was never running.** `nova_atomic_cas_i32` returned the expected
  ...
  *expected* value, so CAS reported success only when that value was odd). That made
  `atomic.nova`'s `test_atomic_i32` fail, and `nova_test_fail` `_Exit(1)`s — aborting the suite
  before `test_atomic_i64`, `test_atomic_bool` and `test_fiber_execution` ever ran. `spawn`, closure
  capture and the arena were all innocent. Now gated by `conformance/cases/31_atomics.nova` (shown
  to fail before the fix), and the harness names the failing test (`nova_test_begin`). Fixed
  alongside: `nova_atomic_cas_i64` declared `int32_t desired` while codegen passes i64.
...
- **`channel.nova`** still has a transitive `Identifier 'fiber' not found` when imported (why
  asyncchan is a separate module). Unrelated to the above; still open.
- ⚠️ **`nova_chan_new` ignores its capacity** — `(void)capacity; // unbounded for now` — and there is
...
- **build.zig reliability**: `zig build` doesn't reliably re-run the opaque runtime-
  ...
  `clang++ -std=c++20 -O2 -pthread -DNOVA_DROP_ARENA -DNOVA_HAVE_WOLFSSL
  -Ideps/wolfssl -Ideps/wolfssl/build -c -I/opt/homebrew/include
  src/runtime/runtime.cpp -o ~/.nova/lib/nova_runtime.o && ar rcs
  ~/.nova/lib/libnova_runtime.a ~/.nova/lib/nova_runtime.o`. Consider making the
