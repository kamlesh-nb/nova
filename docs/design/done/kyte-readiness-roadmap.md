# Kyte Language — Production-Readiness Roadmap
...
> - **A8 "add string interpolation" + `format(fmt, ...)`** — Kyte has had ES6 template strings all
>   along; `format` was explicitly rejected (evolution plan L2).
> - **A3 "arg-count check blocked"** — false, it is enforced (`wrong_arg_count.ky`).
...
Master plan to take Kyte from its current state to a production-usable language: stackful C++20
...
## 1. Where Kyte is today (audited, honest grades)
...
| Crypto | **Fake** | `kyte_sha256`/`kyte_md5` are stubs returning `""` while their `@test` asserts a real digest; wolfSSL is vendored+linked but unused (→ C6) |
...
- **A1. Real closure environments.** ✅ **IMPLEMENTED.** Replaced `captured_globals`
  ...
  `conformance/cases/06_closures_advanced.ky` (returned closures independent, loop-capture
  independent, multi-variable capture) + `04`/`05` regression guards — 7/7 corpus green.
  **Remaining (follow-up):** env/box use `kyte_bytes_alloc_persistent` (they leak, same as the old
  ...
  - *(historical)* **Confirmed bug** (was `conformance/acceptance_a1/closure_capture.ky`, now
    promoted into `06_closures_advanced.ky`):
  ...
  - Guarded by `conformance/cases/04_closures.ky` + `05_closures_capture.ky` (must stay green).
- **A2. Generics through the AST → monomorphization.**
  - ✅ **Increment 1 (done): carry type params through the AST.** `FunctionDecl`/`StructDecl` now
    ...
    `conformance/cases/07_generics.ky` (multi-param `Pair<A,B>` with mixed fields + generic
  ...
  - **Increment 2 (next): use the type params — LARGER than it looks; scope carefully.**
    ...
    - **`size_of<T>` / unboxing / real monomorphization** requires *adding a monomorphization pass*
      ...
      quick win. Note `array.ky` is **dead code** (unused), so its `elem_size=4` bug is moot —
    ...
    - ✅ **Step (a) done: negative-test harness.** `conformance/expect_fail/*.ky` must fail to
...
- **A3. Sound(er) type checker.** Arg count/type checking, return-type checking, condition-is-bool,
  ...
  - ✅ **Started:** resolver now types comparison/logical operators as `bool` (were `i32`), and a
    **condition-must-be-bool** check rejects non-bool `if`/`while` conditions
    (`expect_fail/non_bool_condition.ky`). Corpus 14/14, no stdlib false positives.
...
- **A9. Parameter passing — `ref` params (and an honest pointer type).** *(Requested 2026-07-15.
  ...
  - **Today:** structs are heap+ARC objects, so they *already* pass by reference (mutating a param's
    ...
    addresses in `i32` fields (`bytes.alloc` → `Reader.buf: i32` in `data/btree/client.ky`), which
...
the TLS-verify hole, adds `async`/`await` as sugar, and switches to a prebuilt `libkyte_runtime.a`.
...
- **C6. Crypto — expose wolfCrypt properly.** *(Requested 2026-07-15. Verdict: DO IT; best
  ...
  - **Today `crypto` is fake.** `runtime/core.cpp:202` — `kyte_sha256` and `kyte_md5` **return the
    empty string**:
    `char *kyte_sha256(const char *input) { return kyte_from_cstr(input ? "" : ""); }`.
    `crypto.ky`'s own `@test` asserts `sha256("hello") == "2cf24dba…"`, so that test has been
    passing vacuously or never running. Meanwhile **wolfSSL is already vendored, built and linked**
    (`deps/wolfssl/build/libwolfssl.a`, `-DKYTE_HAVE_WOLFSSL`) — wolfCrypt is sitting there unused.
  - **⚠️ Prerequisite (real blocker): binary-safe strings.** `kyte_from_cstr` is **NUL-terminated**, so
    ...
    length-aware constructor (`kyte_from_bytes(ptr, len)`) writing the 8-byte header directly. Land
  ...
  - **Work.** New `runtime/crypto.cpp` in the unity build, included **after `io.cpp`** (which already
    sequences wolfSSL's `options.h` vs Asio); every entry guarded by `KYTE_HAVE_WOLFSSL` with an honest
    **compile/run error** when absent — *never* a silent empty-string stub again. Then `std/crypto/*.ky`
...
- **C7. `text` namespace — string, stringBuilder, regex, localization.** *(Requested 2026-07-15.
  ...
  - **Regex — build it in Kyte, as a Thompson NFA.** Options weighed: bind PCRE2/RE2 (new heavy C dep,
...
- **Deferred (not a priority): the app/composition layer (`app.ky`).** It needs a redesign
...
- **E2. Conformance test corpus** pinning codegen behavior — the safety net that lets B/C/D proceed
  without regressions. (Leverage existing `kyte test` + coverage.)
...
2. **M2 — Type system & generics.** A2, A3, A4, A6. *The core becomes sound and generics real.*
   ...
   goal): **A2(d) monomorphization** → future perf track (generics work via erasure; array.ky is
...
3. **M3 — C++20 runtime + async.** *(Architecture changed — see `runtime-cpp20-plan.md`.)*
   - ✅ **v0 DONE:** C++20 runtime live, build restored, **corpus 18/18 green**. ABI frozen
     (`kyte_abi.h`); arena-faithful ARC allocator; core subsystems; real file/dir (`std::filesystem`);
     cross-platform (`#ifdef _WIN32` guards); **synchronous concurrency shim** (stable default);
     runtime **prebuilt** into `~/.kyte/lib/libkyte_runtime.a` (fast per-build linking).
   ...
   - **Gate M0:** wire a middle-end pass run (`LLVMRunPasses`) into the emit path so `llvm.coro.*`
     lowers; prove one Kyte coroutine suspends/resumes via Asio.
...
> **Full detail + worked code examples: [`kyte-language-evolution-plan.md`](./kyte-language-evolution-plan.md)**
...
1. **The string heap bug is live.** ASAN puts a wild read in `string_slice` (a string whose length
   ...
   `lang/repro/driver_alloc_churn_crash.ky`. This is very likely M3's *"Prereq A: make strings
   ARC-correct"* biting for real. **Fix before building more on strings** — A8/C6/C7 all pile onto
   exactly that code path.
2. **Binary-safe strings** (`kyte_from_bytes(ptr,len)`) — gates C6 and every binary protocol.
...
**After completing all of the above, Kyte would be a solid Beta.** Concretely that means:
...
integration, YCSB benchmarks, and real Kyte web apps all become realistic.
...
**Bottom line:** this roadmap converts Kyte from "impressive alpha with an unsound core and unsafe
