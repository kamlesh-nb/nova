...
| W11 | Codegen soundness fuzzer (gen well-typed → compile → run → oracle) | 🟡 | **Landed + broadened** `conformance/codegen_fuzz.py` + `.sh`: generates well-typed **int and long** expressions (`+ - * / % & \| << >>`), computes the exact answer with Nova's semantics (32/64-bit wrap, trunc div/mod, narrowing casts), and the **six comparisons** (value shape `assert.equalInt`; boolean shape `assert.isTrue/isFalse`, which also value-checks long). Compiles+runs, flags MISCOMPILE on mismatch. **Teeth proven** (wrong oracle flagged, both value and compare shapes). All match on the fixed compiler across seeds. | next: structs/generics/optionals/ownership (run under ASAN); wire into CI at scale |
