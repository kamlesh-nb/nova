...
written in **Zig**. It is the storage engine behind the Kyte ecosystem, yet it stands alone as a database
in its own right. Kyte connects to it over a binary wire protocol; NovaDB is intentionally a *separate*
project, and is to be built and versioned separately (it resides in `../../btree/`, not in this
repository).

> This is a navigable overview in the manner of the Kyte architecture set. For the authoritative design
...
wire protocol** (`proto/`) is the intended path for Kyte's driver.
...
Kyte (the `lang` repository) talks to NovaDB through this binary protocol, by way of the published
**`nova-novadb`** driver package, which implements the `db` seam (see [04-stdlib.md](04-stdlib.md)). From
Kyte's side, NovaDB is simply one interchangeable backend behind the `Driver` and `Connection` traits; a
program may swap it for Postgres, MySQL, MSSQL, or MongoDB without touching the call sites. NovaDB, for its
part, remains independent, and knows nothing of Kyte beyond the wire.
...
that `kyte.db` is a test artifact (regenerated by running the engine), and is not to be hand edited. The
