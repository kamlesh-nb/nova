...
2. **The conformance corpus is the gate.** `conformance/run.sh` must be green both before and after. The
   positive cases run via `kyte test`; the negatives reside under `expect_fail/`, and must be *rejected for
   the declared reason* (a segfault is not a rejection). A gate is to be added for every behaviour that you
   add.
3. **Verify memory with ASAN, and not merely with ARC.** Run `KYTE_ASAN=1 zig build`, and thereafter
...
- **Gate.** A `conformance/cases/NN_<name>.ky` proving the behaviour, along with an `expect_fail/` case
...
`formatter.zig` as well, so that `kyte fmt` handles it.
...
### A new standard library module (in Kyte)
1. Write `src/std/<area>/<mod>.ky`.
...
3. Add a conformance case. Kindly note that **uncovered standard library modules rot silently**; a module
   that no one imports can cease to compile unnoticed. Compile check it with `kyte <file-that-imports-it> -o
   out` (and not merely with `kyte test`), and grep for `.ky:L:C:` errors.
...
not by string matching a type name. When in doubt, run `KYTE_SEMA_SHADOW=1` to diff the two type engines.
...
KYTE_ASAN=1 zig build && conformance/run.sh --asan   # the memory gate, for allocation and ownership changes
# 2. Add the gate(s): conformance/cases/NN_*.ky (along with expect_fail/*.ky for rejections).
