#!/usr/bin/env bash
# Assemble a runnable Nova distribution: the `nova` COMPILER plus the `.nova` home tree it
...
# the compiler you actually invoke on a .nova file.
#
# WHY THIS IS HOST-ONLY, and what it would take to change that
# ------------------------------------------------------------
# `nova` links LLVM itself (LLVM-C dynamically, or the static component libs) out of
# NOVA_LLVM_PREFIX. Cross-building the compiler therefore needs LLVM built FOR THE TARGET,
...
#
# Note this does NOT block producing binaries for other architectures: `nova app.nova
...
#
# To add an arm64 host: install or build LLVM for it, point NOVA_LLVM_PREFIX at that, and
...
EXE=nova; [[ "$os" == windows-msvc ]] && EXE=nova.exe
...
rm -rf "$OUT"
mkdir -p "$OUT/bin" "$OUT/nova-home/lib" "$OUT/nova-home/src"

cp "zig-out/bin/$EXE" "$OUT/bin/$EXE"
cp -r src/lib/std   "$OUT/nova-home/std"
cp -r src/runtime   "$OUT/nova-home/src/runtime"
cp -r deps          "$OUT/nova-home/deps"

# The runtime archive/objects for THIS host. build-archives.sh writes per-target copies under
# zig-out/archives/<triple>/; fall back to the ones `zig build` installed into ~/.nova/lib.
if [[ -d "zig-out/archives/$TRIPLE" ]]; then
  cp zig-out/archives/"$TRIPLE"/*.a zig-out/archives/"$TRIPLE"/*.lib zig-out/archives/"$TRIPLE"/*.o "$OUT/nova-home/lib/" 2>/dev/null
else
  cp "$HOME"/.nova/lib/* "$OUT/nova-home/lib/" 2>/dev/null
fi

cat > "$OUT/INSTALL.txt" <<EOF
Nova distribution -- $TRIPLE

  bin/$EXE      the compiler
  nova-home/    its runtime home: stdlib source, runtime archive, runtime C++ source, deps

The compiler resolves its home as \$HOME/.nova, so install with:

  cp -r nova-home/*  \$HOME/.nova/
  cp bin/$EXE        \$HOME/.nova/bin/

Then:  nova yourfile.nova -o yourprog

Cross-compiling to another architecture does NOT need a compiler built for it:

  nova yourfile.nova --target linux-arm64    -o prog
  nova yourfile.nova --target windows-arm64  -o prog.exe
...
echo "  nova-home $(du -sh "$OUT/nova-home" | cut -f1)"
